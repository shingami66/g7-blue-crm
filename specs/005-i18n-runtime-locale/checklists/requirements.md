...
- **Implementation status (docs sync):** Core authenticated surfaces + intentional P3 Settings/Admin localization are implemented in the working tree. Independent review PASS (no P0). Automated i18n/export tests 223/223; root/public alignment tests 21/21. **T032 Mozfer browser smoke remains HOLD.** PDF/document localization, Clerk sign-in/up widgets, and inventing Invoices export remain excluded. C01 Settings/Admin exclusion was later superseded for UI copy only by approved P3 tasks—not by PDF/VAT scope expansion.
