...
**Input**: User-approved product design for a global `Create Invoice` CTA that first asks the user to choose `Create Deposit Invoice` or `Create Final Invoice`, then opens a type-specific eligible-Service selector and navigates only to `/services/{encoded-service-id}?invoiceAction=deposit` or `/services/{encoded-service-id}?invoiceAction=final`. The implementation conceptually constructs the path from `encodeURIComponent(serviceId)`.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - Start invoice creation from the global Invoices page (Priority: P1)

An authorized user starts from the global Invoices page, chooses Deposit or Final, selects an eligible Service for that invoice type, and reaches the existing Service-scoped billing action without creating an Invoice from the chooser.

**Why this priority**: This is the core product change. Without it, the feature provides no value.

**Independent Test**: Can be fully tested by opening the global Invoices page, choosing an invoice type, selecting an eligible Service, and confirming navigation to the existing Service Detail billing action with no mutation triggered from the chooser.

**Acceptance Scenarios**:

1. **Given** a user with `invoices:write` and `services:read`, **When** the user clicks `Create Invoice`, chooses `Create Deposit Invoice`, and selects an eligible Service, **Then** the app navigates to `/services/{encoded-service-id}?invoiceAction=deposit` and the Service Detail page scrolls to and focuses the deposit billing action.
2. **Given** a user with `invoices:write` and `services:read`, **When** the user clicks `Create Invoice`, chooses `Create Final Invoice`, and selects an eligible Service, **Then** the app navigates to `/services/{encoded-service-id}?invoiceAction=final` and the Service Detail page scrolls to and focuses the final billing action.
3. **Given** the chooser flow is open, **When** the user dismisses it, **Then** no Invoice is created and no customer, amount, VAT, payment, discount, item, or other financial payload is submitted.

---

### User Story 2 - Keep invoice authority on the existing Service-scoped flow (Priority: P1)

The new chooser remains read-only and navigation-only while the existing Service Detail billing flow stays the sole invoice-creation authority.
...
**Independent Test**: Can be fully tested by inspecting the chooser submission contract and then confirming stale or changed Service state is rejected by the existing Service Detail and invoice action revalidation path.
...
3. **Given** the user chooses `Create Final Invoice`, **When** the destination flow loads, **Then** current runtime source truth is preserved: Final Invoice eligibility does not require an existing Deposit Invoice or Deposit Payment, and final amount remains server-derived from remaining billable balance.
...
### User Story 3 - Use the chooser reliably across accessibility, mobile, and RTL layouts (Priority: P2)

The chooser flow stays usable for keyboard, mobile, and RTL users while preserving clear invoice-type separation and safe identifier rendering.

**Why this priority**: The feature introduces a two-step modal flow on a core billing page and must remain operable in the repository’s existing English/Arabic UI.

**Independent Test**: Can be fully tested by keyboard-only navigation, mobile-width layout inspection, and RTL rendering checks without creating invoices.

**Acceptance Scenarios**:

1. **Given** the invoice-type chooser or eligible-Service selector is open, **When** the user navigates by keyboard, **Then** focus is trapped correctly, Escape closes the active dialog when appropriate, and focus returns to the opener.
2. **Given** RTL text content and Service identifiers are shown together, **When** the selector renders a Service card, **Then** human text follows logical RTL/LTR layout while identifiers remain LTR-safe and readable.
3. **Given** the eligible-Service list is long, **When** the user is on a mobile viewport, **Then** the two-step chooser remains scrollable, readable, and tappable without horizontal overflow.

The initial implementation uses local search and local pagination with stable deterministic ordering by `service_number` ascending, then `id` ascending. Changing search resets pagination to page 1. Searchable fields are limited to Service number, Service title, customer display text, event name, event date text, and event location. Server-side search and cursor pagination remain deferred. Deposit and Final have separate type-specific empty-state wording. Initial focus moves to the search field when the selector opens. Escape closes the dialog. Focus remains contained and returns to the original opener using a stable opener reference. Mobile layout remains inside the viewport. Stored business text uses `dir="auto"`, Service identifiers use LTR isolation, and dates use bidi-safe presentation. Deposit and Final titles, descriptions, actions, empty states, and no-match states have distinct English and Arabic copy.
...
- What happens when the destination Service page receives an unknown or inapplicable `invoiceAction` query parameter?
...
- **FR-006**: Deposit selection MUST navigate only to `/services/{encoded-service-id}?invoiceAction=deposit`.
- **FR-007**: Final selection MUST navigate only to `/services/{encoded-service-id}?invoiceAction=final`.
- **FR-008**: The Service Detail page MUST interpret `invoiceAction` only as a UI intent hint, scroll to the relevant billing action, and focus the relevant action without widening mutation authority.
- **FR-009**: Existing Deposit and Final invoice actions on the Service Detail page MUST remain the sole mutation authorities.
...
- **FR-017**: The chooser and selector MUST support accessible dialog semantics, keyboard containment, focus return, mobile-friendly layout, `dir="auto"` for stored text, and LTR-safe rendering for identifiers.
...
- **Invoice Action Hint**: A route-level query parameter (`invoiceAction=deposit|final`) used only to scroll to and focus the corresponding Service Detail billing action.

## Success Criteria *(mandatory)*

### Measurable Outcomes

- **SC-001**: An authorized user can start from the global Invoices page and reach the correct Service Detail billing action for Deposit or Final in one chooser flow without any invoice mutation occurring before the existing Service-scoped action.
...
- **SC-003**: Stale or changed Service state is still rejected by the existing destination route and invoice action authority without creating an Invoice.
- **SC-004**: Keyboard, mobile, and RTL checks confirm the chooser flow remains usable and preserves readable identifier directionality.
...
- Deposit and Final invoice actions already exist on the Service Detail page and remain authoritative.
...
- A future implementation may add new UI components and tests, but no database, RPC, schema, RLS, VAT-mode, or accounting redesign belongs to this feature.
