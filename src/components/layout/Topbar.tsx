...
import BusinessYearSelector from "@/components/i18n/BusinessYearSelector";
...
export default function Topbar({
  businessYearOptions,
  businessYearPreference,
}: {
  businessYearOptions: readonly number[];
  businessYearPreference: number;
}) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center justify-between gap-3 border-b border-outline-variant bg-surface px-4 sm:px-6">
      ...
      <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
        ...
        {isSignedIn ? <BusinessYearSelector years={businessYearOptions} preferredYear={businessYearPreference} /> : null}
