...
export function LocaleSelector() {
  ...
  const [isAwaitingLocale, setIsAwaitingLocale] = useState(false);
    ...
    startTransition(async () => {
      try {
        ...
        if (decision.shouldRefresh) {
          // Hold the localized status until providerLocale reflects the requested locale.
          awaitingLocaleRef.current = requestedLocale;
          setIsAwaitingLocale(true);
          router.refresh();
        ...
        // No refresh path (error, or session-pending): clear status safely now.
        clearPendingSafely();
      } catch {
        clearPendingSafely();
      }
  ...
  function handleToggle() {
    if (isPending || isAwaitingLocale || submissionInFlight.current) {
      return;
    }
  ...
  return (
    <div className="relative flex max-w-[11rem] flex-col items-stretch sm:max-w-none">
      <button
        type="button"
        aria-busy={isPending || isAwaitingLocale || undefined}
        ...
        disabled={isPending || isAwaitingLocale}
      ...
      <span aria-live="polite" className="sr-only" id={statusId}>
        {isPending || isAwaitingLocale ? copy.updating : ""}
      </span>
