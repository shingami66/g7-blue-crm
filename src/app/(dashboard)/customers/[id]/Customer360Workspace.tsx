    ...
    <div className="space-y-5">
      ...
      <section id="customer360-overview" className="scroll-mt-20 space-y-3">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <SummaryCard label={dictionary.summary.upcoming} value={String(data.upcomingServices.length)} />
        </div>
      </section>
      ...
      <section id="customer360-services" className="scroll-mt-20 rounded-xl border border-surface-variant bg-surface-container-lowest p-5">
        ...
        {data.upcomingServices.length === 0 ? <EmptyState text={dictionary.states.noUpcoming} /> : <div className="grid grid-cols-1 gap-3 md:grid-cols-2">{data.upcomingServices.map((service) => <PendingLink key={service.id} href={`/services/${service.id}`} className="flex min-w-0 items-center justify-between gap-3 rounded-lg border border-outline-variant p-3 hover:bg-surface-container-low"><span className="min-w-0"><span className="block font-semibold text-primary" dir="ltr">{isolateBidiText(service.serviceNumber)}</span><span className="block truncate text-[14px] text-on-surface" dir="auto">{isolateBidiText(service.serviceTitle)}</span></span><span className="shrink-0 text-[12px] text-on-surface-variant" dir="ltr"><UiDateText locale={locale} value={service.eventStartDate ?? ""} /></span></PendingLink>)}</div>}
      </section>
      ...
      <details id="customer360-activity" open className="scroll-mt-20 space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2"><ActivitySection title={dictionary.sections.operationalActivity} activities={data.recentOperationalActivity} locale={locale} dictionary={dictionary} /><ActivitySection title={dictionary.sections.financialActivity} activities={data.recentFinancialActivity} locale={locale} dictionary={dictionary} /></div>
      </details>
    </div>
  );
}
