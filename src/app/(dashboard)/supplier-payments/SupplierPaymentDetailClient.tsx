...
export default function SupplierPaymentDetailClient({ payment, canReverse, dictionary }: { payment: SupplierPaymentDetail; canReverse: boolean; dictionary: SupplierPaymentsDictionary }) {
  ...
  return (
    <div dir={isRtl ? "rtl" : "ltr"} className="mx-auto flex w-full max-w-6xl min-w-0 flex-col gap-5 pb-12" data-supplier-payment-detail="workspace">
      ...
      {payment.status === "reversed" && <InfoSection title={dictionary.fields.reversalAudit}><Info label={dictionary.fields.reversalReason} value={payment.reversal_reason} /><Info label={dictionary.fields.reversedAt}><UiDateTimeText locale={locale} value={payment.reversed_at} /></Info><Info label={dictionary.fields.reversedBy} value={payment.reversed_by_name ?? dictionary.states.unknownUser} /></InfoSection>}
