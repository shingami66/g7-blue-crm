...
function ReceiptCorrectionActions({ receipt, dictionary }: { receipt: ApprovedCommitment["receipts"][number]; dictionary: ProcurementCommitmentDictionary }) {
  ...
  useEffect(() => {
    ...
    if (!isOpen && wasOpen.current) document.getElementById(`receipt-correction-trigger-${receipt.id}`)?.focus();
    wasOpen.current = isOpen;
  }, [isOpen, receipt.id]);
  function correct() {
    startTransition(async () => {
      const result = await correctServiceReceipt({ receiptId: receipt.id, correctedAcceptanceStatus: status, correctedReceivedAmount: amount.trim() ? Number(amount) : null, correctedConditionsNotes: status === "ACCEPTED_WITH_CONDITIONS" ? conditions.trim() || null : null, correctionReason: reason, requestId: requestId(requestRef) });
      if (!result.success) { setMessage(errorText(dictionary, result.code)); return; }
      resetRequestId(requestRef); setMessage(dictionary.success.receiptCorrected); setReason(""); setIsOpen(false); router.refresh();
    });
  }
  return <div className="mt-4"><Button id={`receipt-correction-trigger-${receipt.id}`} type="button" size="sm" variant="secondary" onClick={() => setIsOpen((open) => !open)} aria-expanded={isOpen} aria-controls={isOpen ? `receipt-correction-${receipt.id}` : undefined}>{isOpen ? dictionary.forms.correctionTitle : dictionary.forms.correctReceipt}</Button>{isOpen && <div id={`receipt-correction-${receipt.id}`} className="mt-3 rounded-lg border border-warning/50 bg-warning-container/30 p-3"><p className="text-[12px] font-semibold text-on-surface">{dictionary.forms.correctionTitle}</p><div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-3"><label className="text-[12px] font-semibold">{dictionary.fields.correctedOutcome}<select ref={firstFieldRef} className={inputClass} value={status} onChange={(event) => setStatus(event.target.value as typeof status)} disabled={isPending}><option value="ACCEPTED">{dictionary.forms.accepted}</option><option value="ACCEPTED_WITH_CONDITIONS">{dictionary.forms.conditional}</option><option value="REJECTED">{dictionary.forms.rejected}</option></select></label><label className="text-[12px] font-semibold">{dictionary.fields.receivedAmount}<input className={inputClass} type="number" min="0" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder={dictionary.forms.optionalPlaceholder} disabled={isPending} /></label><fieldset className="md:col-span-1"><legend className="text-[12px] font-semibold">{dictionary.fields.correctionNoteReason}</legend><div className="mt-1 space-y-2"><label className="block text-[12px] font-semibold">{dictionary.fields.correctedConditions}<textarea className={textAreaClass} value={conditions} onChange={(event) => setConditions(event.target.value)} required={status === "ACCEPTED_WITH_CONDITIONS"} rows={2} disabled={isPending} /></label><label className="block text-[12px] font-semibold">{dictionary.fields.correctionReason}<textarea className={textAreaClass} value={reason} onChange={(event) => setReason(event.target.value)} placeholder={dictionary.forms.correctionReasonPlaceholder} required rows={2} disabled={isPending} /></label></div></fieldset></div><div className="mt-2 flex flex-wrap items-center gap-2"><Button type="button" size="sm" variant="secondary" onClick={correct} disabled={isPending || !reason.trim() || status === "ACCEPTED_WITH_CONDITIONS" && !conditions.trim()}>{isPending ? "…" : dictionary.forms.correct}</Button><Button type="button" size="sm" variant="secondary" onClick={() => setIsOpen(false)} disabled={isPending}>{dictionary.forms.cancel}</Button>{message && <span role="status" className="text-[12px] text-on-surface-variant">{message}</span>}</div></div>}</div>;
}
