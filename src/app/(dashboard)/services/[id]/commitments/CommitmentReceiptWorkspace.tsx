      ...
      <div className="space-y-5 p-5">
        ...
        <details className="rounded-lg border border-outline-variant/70 bg-surface-container-low">
          ...
          <div className="space-y-3 border-t border-outline-variant/70 p-4">
            {commitment.receipts.length > 0 ? commitment.receipts.map((receipt) => <ReceiptRecord key={receipt.id} receipt={receipt} canAccept={canAcceptReceipt} canCorrect={canCorrectReceipt} canUploadDocuments={canUploadDocuments} dictionary={dictionary} />) : <p className="text-[13px] text-on-surface-variant">{dictionary.notices.noReceipts}</p>}
        ...
        {commitment.status === "open" && canTransition && (
          <details className="rounded-lg border border-outline-variant/70 bg-surface-container-low">
            <summary className="cursor-pointer list-none px-4 py-3 text-[14px] font-semibold text-primary">{dictionary.forms.close} / {dictionary.forms.cancel}</summary>
            <div className="border-t border-outline-variant/70 p-4"><LifecycleForm commitmentId={commitment.id} dictionary={dictionary} /></div>
          </details>
        )}
