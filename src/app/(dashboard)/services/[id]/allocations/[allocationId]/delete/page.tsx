...
export default async function DeleteSupplierAllocationPage({
  ...
  const dictionary = servicesDictionary.supplierAllocations.subflow.deletePage;
  ...
  if (allocation.isDeleted) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        <div className="w-full max-w-md p-8 bg-white rounded-xl border border-slate-200 shadow-sm text-center">
          <h2 className="text-xl font-semibold text-slate-900 mb-2">
            {dictionary.alreadyDeletedTitle}
          </h2>
          <p className="text-sm text-slate-500">{dictionary.alreadyDeletedMessage}</p>
          <Link
            href={`/services/${id}`}
            className="mt-6 inline-flex items-center gap-2 text-primary hover:underline font-medium"
          >
