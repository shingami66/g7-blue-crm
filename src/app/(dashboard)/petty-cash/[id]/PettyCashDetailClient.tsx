...
export default function PettyCashDetailClient({ canRead, canManage, canTransact, fund, transactions, expenses, custodians, loadError }: { canRead: boolean; canManage: boolean; canTransact: boolean; fund: PettyCashFund | null; transactions: PettyCashTransaction[]; expenses: PettyCashExpenseSummary[]; custodians: PettyCashCustodianOption[]; loadError: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="space-y-3"><h2 className="text-base font-semibold text-on-surface">{dictionary.labels.transactionLedger}</h2>{transactions.length === 0 ? <Empty text={dictionary.labels.noFundsDescription} /> : <div className="space-y-2">{transactions.map((tx) => <TransactionRow key={tx.id} transaction={tx} dictionary={dictionary} expenseNumber={tx.expense_id ? expenseNumberById.get(tx.expense_id) : undefined} />)}</div>}</section>
...
function TransactionRow({ transaction, dictionary, expenseNumber }: { transaction: PettyCashTransaction; dictionary: ReturnType<typeof getPettyCashDictionary>; expenseNumber?: string }) { return <div className="grid gap-2 rounded-xl border border-outline-variant bg-surface-container-lowest p-3 text-sm md:grid-cols-8 md:items-center"><div className="font-semibold md:col-span-2">{dictionary.transactionTypes[transaction.transaction_type]}</div><div className="font-mono" dir="ltr">{money(transaction.amount)}</div><div className="font-mono text-on-surface-variant" dir="ltr">{money(transaction.balance_before)} → {money(transaction.balance_after)}</div><div className="font-mono" dir="ltr">{expenseNumber ?? "—"}</div><div className="text-on-surface-variant">{transaction.reference ?? "—"}</div><div className="text-on-surface-variant"><bdi dir="ltr">{formatTransactionDateTime(transaction.recorded_at)}</bdi></div><div className="text-on-surface-variant">{transaction.notes ?? "—"}</div></div>; }
