...
import { approveAndDisbursePettyCashExpenseAction, attachExpenseReceiptAction, recordPettyCashExpenseAction, recordPettyCashTransactionAction, setPettyCashFundStatusAction, updatePettyCashFundAction } from "@/lib/expenses/actions";
...
type PettyCashActionPanel = "manage" | "replenish" | "expense" | "withdraw";
...
export default function PettyCashDetailClient({ canRead, canManage, canTransact, fund, transactions, expenses, custodians, loadError }: { canRead: boolean; canManage: boolean; canTransact: boolean; fund: PettyCashFund | null; transactions: PettyCashTransaction[]; expenses: PettyCashExpenseSummary[]; custodians: PettyCashCustodianOption[]; loadError: boolean }) {
  ...
  const [approveRequest, setApproveRequest] = useState<{ expenseId: string; requestId: string } | null>(null);
  ...
  const closePanel = () => { setPanel(null); setRequestId(null); setError(null); setContextType("company"); setServiceId(""); };
  ...
  const submitTransaction = (event: React.FormEvent<HTMLFormElement>, transactionType: "replenishment" | "treasury_withdrawal") => {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    startTransition(async () => {
      const result = await recordPettyCashTransactionAction({ fund_id: fund.id, transaction_type: transactionType, amount: Number(data.get("amount")), reference: data.get("reference") || null, expense_id: null, notes: data.get("notes") || null, request_id: requestId });
  ...
  const approveAndDisburse = (expenseId: string) => {
    const request = approveRequest?.expenseId === expenseId ? approveRequest : { expenseId, requestId: crypto.randomUUID() };
    setApproveRequest(request);
    setError(null);
    startTransition(async () => {
      const result = await approveAndDisbursePettyCashExpenseAction({ fund_id: fund.id, expense_id: expenseId, request_id: request.requestId });
      if (!result.success) { setError(result.error ?? dictionary.states.error); return; }
      finish();
    });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="space-y-3"><h2 className="text-base font-semibold text-on-surface">{dictionary.labels.pettyCashExpenses}</h2>{expenses.length === 0 ? <Empty text={dictionary.forms.noApprovedExpenses} /> : <><div className="grid gap-3 md:hidden" data-testid="mobile-petty-cash-expense-cards">{expenses.map((expense) => <PettyCashExpenseCard key={expense.id} expense={expense} dictionary={dictionary} canApproveAndDisburse={canTransactFund} onApproveAndDisburse={approveAndDisburse} isPending={isPending} />)}</div><div className="hidden overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest md:block" data-testid="desktop-petty-cash-expense-table"><table className="w-full table-fixed text-start text-sm"><colgroup><col className="w-[24%]" /><col className="w-[14%]" /><col className="w-[18%]" /><col className="w-[14%]" /><col className="w-[14%]" /><col className="w-[16%]" /></colgroup><thead className="border-b border-outline-variant bg-surface-container-low"><tr><th className="px-3 py-2 text-start">{dictionary.labels.linkedExpense}</th><th className="px-3 py-2 text-start">{dictionary.labels.date}</th><th className="px-3 py-2 text-start">{dictionary.labels.category}</th><th className="px-3 py-2 text-end">{dictionary.labels.amount}</th><th className="px-3 py-2 text-start">{dictionary.labels.status}</th><th className="px-3 py-2 text-end">{dictionary.labels.action}</th></tr></thead><tbody>{expenses.map((expense) => { const isDisbursed = expense.status === "approved" && expense.remaining_petty_cash_amount <= 0; const canApprove = canTransactFund && !isDisbursed && (expense.status === "submitted" || expense.status === "approved") && expense.remaining_petty_cash_amount > 0; return <tr key={expense.id} className="border-b border-outline-variant last:border-0"><td className="px-3 py-2 text-start font-mono font-semibold"><bdi dir="ltr">{expense.expense_number}</bdi></td><td className="px-3 py-2 text-start"><bdi dir="ltr">{expense.expense_date}</bdi></td><td className="px-3 py-2 text-start break-words">{expense.expense_category}</td><td className="px-3 py-2 text-end font-mono"><bdi dir="ltr">{money(expense.amount)}</bdi></td><td className="px-3 py-2 text-start">{isDisbursed ? dictionary.labels.disbursed : dictionary.statuses[expense.status as keyof typeof dictionary.statuses] ?? expense.status}</td><td className="px-3 py-2 text-end">{isDisbursed ? <span className="font-semibold text-emerald-700">{dictionary.labels.disbursed}</span> : canApprove ? <button type="button" onClick={() => approveAndDisburse(expense.id)} disabled={isPending} className="rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-on-primary disabled:opacity-50">{isPending && approveRequest?.expenseId === expense.id ? dictionary.actions.working : dictionary.actions.approveAndDisburse}</button> : <span className="text-on-surface-variant">—</span>}</td></tr>; })}</tbody></table></div></>}</section>
...
function PettyCashExpenseCard({ expense, dictionary, canApproveAndDisburse, onApproveAndDisburse, isPending }: { expense: PettyCashExpenseSummary; dictionary: ReturnType<typeof getPettyCashDictionary>; canApproveAndDisburse: boolean; onApproveAndDisburse: (expenseId: string) => void; isPending: boolean }) { const isDisbursed = expense.status === "approved" && expense.remaining_petty_cash_amount <= 0; const canApprove = canApproveAndDisburse && !isDisbursed && (expense.status === "submitted" || expense.status === "approved") && expense.remaining_petty_cash_amount > 0; const status = isDisbursed ? dictionary.labels.disbursed : dictionary.statuses[expense.status as keyof typeof dictionary.statuses] ?? expense.status; return <article className="min-w-0 rounded-xl border border-outline-variant bg-surface-container-lowest p-4" data-testid="mobile-petty-cash-expense-card"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs text-on-surface-variant">{dictionary.labels.linkedExpense}</p><p className="mt-1 break-all font-mono font-semibold text-on-surface" dir="ltr">{expense.expense_number}</p></div><span className="shrink-0 rounded-full border border-outline-variant px-2 py-1 text-xs font-semibold">{status}</span></div><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><Detail label={dictionary.labels.date} value={<span dir="ltr">{expense.expense_date}</span>} /><Detail label={dictionary.labels.category} value={expense.expense_category} /><Detail label={dictionary.labels.amount} value={<span dir="ltr" className="font-mono">{money(expense.amount)}</span>} /><Detail label={dictionary.labels.remaining} value={<span dir="ltr" className="font-mono">{money(expense.remaining_petty_cash_amount)}</span>} /></dl><div className="mt-4 flex justify-end">{isDisbursed ? <span className="font-semibold text-emerald-700">{dictionary.labels.disbursed}</span> : canApprove ? <button type="button" onClick={() => onApproveAndDisburse(expense.id)} disabled={isPending} className="rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-on-primary disabled:opacity-50">{dictionary.actions.approveAndDisburse}</button> : <span className="text-on-surface-variant">—</span>}</div></article>; }
