...
import { Building2, FileText, Landmark, MapPin, Pencil, ShieldAlert, UserRound } from "lucide-react";
...
import { getSupplierRecordNavigation, safeRecordReturnTo, appendReturnTo } from "@/lib/record-navigation/queries";
...
export default async function SupplierDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ showDeleted?: string; returnTo?: string }> }) {
  ...
  const currentSupplierUrl = resolvedSearchParams.returnTo
    ? appendReturnTo(`/suppliers/${supplier.id}${isDeleted ? "?showDeleted=true" : ""}`, returnTo)
    : `/suppliers/${supplier.id}${isDeleted ? "?showDeleted=true" : ""}`;
  const recordNavigationDictionary = getRecordNavigationDictionary(locale);

  return <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 pb-12"><div className="flex flex-wrap items-start justify-between gap-4 py-4"><div className="flex items-start gap-3"><PendingLink href={returnTo} className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-outline-variant bg-surface text-on-surface hover:bg-surface-container-low transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40" aria-label={dictionary.detail.backToSuppliers}><LocaleBackIcon size={16} /></PendingLink><div><div className="flex flex-wrap items-center gap-3"><h1 className="text-[28px] font-semibold leading-[36px] text-primary" dir="auto">{supplier.name}</h1><StatusBadge variant={statusVariants[supplier.status]}>{getSupplierStatusLabel(locale, supplier.status)}</StatusBadge>{isDeleted && <span className="rounded bg-error-container px-2 py-1 text-[12px] font-semibold text-error">{dictionary.detail.deleted}</span>}</div>{supplier.supplierNumber && <p className="mt-1 text-[13px] text-on-surface-variant" dir="ltr">{supplier.supplierNumber}</p>}<p className="mt-1 text-[14px] text-on-surface-variant">{dictionary.detail.subtitle}</p></div></div><div className="flex flex-wrap items-center justify-end gap-2"><Suspense fallback={<RecordNavigationPlaceholder recordType={dictionary.list.title} dictionary={recordNavigationDictionary} state="loading" />}><RecordNavigationSlot loadNavigation={() => getSupplierRecordNavigation(id, { isPreferred: supplier.isPreferred, name: supplier.name }, includeDeleted)} basePath="/suppliers" recordType={dictionary.list.title} dictionary={recordNavigationDictionary} returnTo={returnTo} pendingLabel={dictionary.list.openingSupplier} /></Suspense>{canEdit && !isDeleted && <><SupplierBlacklistActions supplier={supplier} dictionary={dictionary.blacklist} /><PendingLink href={`/suppliers/${supplier.id}/edit`} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-outline-variant bg-surface px-3 py-1.5 text-[13px] font-semibold text-on-surface hover:bg-surface-container-low"><Pencil size={15} />{dictionary.detail.edit}</PendingLink></>}{canDelete && <SupplierDeleteRestoreActions supplierId={supplier.id} supplierName={supplier.name} isDeleted={isDeleted} dictionary={dictionary.deleteRestore} />}</div></div>
      ...
      {supplier.canReadBank && <DetailCard title={dictionary.detail.bankDetails} icon={<Landmark size={18} />}><div className="grid grid-cols-1 gap-4 md:grid-cols-3"><DetailValue label={dictionary.detail.bankName} value={supplier.bankName} /><DetailValue label={dictionary.detail.bankAccountName} value={supplier.bankAccountName} /><DetailValue label={dictionary.detail.iban} value={supplier.iban} ltr /></div>{!supplier.bankName && !supplier.bankAccountName && !supplier.iban && <p className="text-[14px] text-on-surface-variant">{dictionary.detail.noBankDetails}</p>}</DetailCard>}
        {canViewCosting && <DetailCard title={dictionary.quotationHistory.title} icon={<FileText size={18} />}><p className="-mt-2 text-[13px] text-on-surface-variant">{dictionary.quotationHistory.subtitle}</p><PendingLink href={appendReturnTo(`/suppliers/${supplier.id}/quotations${isDeleted ? "?showDeleted=true" : ""}`, currentSupplierUrl)} className="inline-flex min-h-9 items-center self-start rounded-lg border border-outline-variant bg-surface px-3 py-1.5 text-[13px] font-semibold text-primary hover:bg-surface-container-low">{dictionary.quotationHistory.openWorkspace}</PendingLink></DetailCard>}
