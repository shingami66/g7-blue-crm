...
export default async function SupplierDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ showDeleted?: string }> }) {
  const [{ id }, { showDeleted }, locale] = await Promise.all([params, searchParams, getCurrentSessionEffectiveLocale()]);
  ...
  const { supplier, canEdit, canDelete, canViewCosting } = data;
  const isDeleted = supplier.isDeleted;

  return <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 pb-12"><div className="flex flex-wrap items-start justify-between gap-4 py-4"><div className="flex items-start gap-3"><PendingLink href={isDeleted ? "/suppliers?showDeleted=true" : "/suppliers"} className="rounded-lg border border-outline-variant bg-surface p-2 text-on-surface hover:bg-surface-container-low" aria-label={dictionary.detail.backToSuppliers}><ArrowLeft size={18} /></PendingLink><div><div className="flex flex-wrap items-center gap-3"><h1 className="text-[28px] font-semibold leading-[36px] text-primary" dir="auto">{supplier.name}</h1><StatusBadge variant={statusVariants[supplier.status]}>{getSupplierStatusLabel(locale, supplier.status)}</StatusBadge>{isDeleted && <span className="rounded bg-error-container px-2 py-1 text-[12px] font-semibold text-error">{dictionary.detail.deleted}</span>}</div>{supplier.supplierNumber && <p className="mt-1 text-[13px] text-on-surface-variant" dir="ltr">{supplier.supplierNumber}</p>}<p className="mt-1 text-[14px] text-on-surface-variant">{dictionary.detail.subtitle}</p></div></div><div className="flex flex-wrap gap-2">{canEdit && !isDeleted && <><SupplierBlacklistActions supplier={supplier} dictionary={dictionary.blacklist} /><PendingLink href={`/suppliers/${supplier.id}/edit`} className="inline-flex items-center gap-2 rounded-lg border border-outline-variant bg-surface px-3 py-2 text-[14px] font-semibold text-on-surface hover:bg-surface-container-low"><Pencil size={16} />{dictionary.detail.edit}</PendingLink></>}{canDelete && <SupplierDeleteRestoreActions supplierId={supplier.id} supplierName={supplier.name} isDeleted={isDeleted} dictionary={dictionary.deleteRestore} />}</div></div>
      ...
      {canViewCosting && !isDeleted && <DetailCard title={dictionary.detail.rateCards} icon={<Landmark size={18} />}><SupplierRateCardsList supplierId={supplier.id} dictionary={dictionary.rateCards} locale={locale} /></DetailCard>}
    </div>;
}

async function loadSupplierDetail(id: string, includeDeleted: boolean) {
  try {
    const [result, canEdit, canDelete, canViewCosting] = await Promise.all([
    ...
    return { kind: "ready" as const, supplier: result.supplier, canEdit, canDelete, canViewCosting };
  } catch (err) {
