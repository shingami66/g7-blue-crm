...
export default function SupplierEditForm({ supplier, dictionary, canManageBankDetails }: { supplier: Supplier; dictionary: SuppliersDictionary; canManageBankDetails: boolean }) {
  ...
  return (
    <div className="flex w-full max-w-5xl flex-col gap-6">
      <div className="flex items-center gap-4 py-4"><button type="button" onClick={() => push(`/suppliers/${supplier.id}`)} className="rounded-lg border border-outline-variant bg-surface p-2 text-on-surface transition-colors hover:bg-surface-container-low" aria-label={dictionary.form.backToSuppliers}><ArrowLeft size={18} /></button><div><h2 className="text-[28px] font-semibold leading-[36px] text-primary">{dictionary.form.editTitle}</h2><p className="text-[14px] text-on-surface-variant">{dictionary.form.editSubtitle}</p></div></div>
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="mt-4 flex justify-end gap-3"><button type="button" onClick={() => push(`/suppliers/${supplier.id}`)} disabled={isSubmitting} className="rounded-lg border border-outline-variant bg-surface px-6 py-2 font-semibold text-on-surface transition-colors hover:bg-surface-container-low disabled:opacity-50">{dictionary.form.buttons.cancel}</button><Button type="submit" loading={isSubmitting}><Save size={18} />{dictionary.form.buttons.update}</Button></div>
      </form>
    </div>
  );
}
