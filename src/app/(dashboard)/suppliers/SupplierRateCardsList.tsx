...
type Draft = { category: string; itemName: string; unit: string; currency: "SAR"; baseCost: string; validFrom: string; validTo: string; status: "active" | "inactive"; notes: string };
const blankDraft: Draft = { category: "", itemName: "", unit: "", currency: "SAR", baseCost: "", validFrom: "", validTo: "", status: "inactive", notes: "" };
...
export default function SupplierRateCardsList({ supplierId, dictionary, locale, canManage }: { supplierId: string; dictionary: SuppliersDictionary["rateCards"]; locale: Locale; canManage: boolean }) {
  ...
  function openEdit(rate: SupplierRateCard) {
    ...
    setDraft({ category: rate.category ?? "", itemName: rate.itemName, unit: rate.unit, currency: "SAR", baseCost: String(rate.baseCost), validFrom: rate.validFrom, validTo: rate.validTo ?? "", status: rate.status, notes: rate.notes ?? "" });
    setFeedback(null);
    setFormOpen(true);
  }
  ...
  function showResult(result: SupplierRateCardActionResult, successMessage: string) {
    ...
    if (result.error === "overlap" && result.conflict) {
      setFeedback(formatSupplierCopy(dictionary.overlap, { ...result.conflict, category: result.conflict.category ?? "", validTo: result.conflict.validTo ?? dictionary.current }));
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    setFeedback(null);
    const base = { supplierId, category: draft.category || null, itemName: draft.itemName, unit: draft.unit, currency: "SAR" as const, baseCost: draft.baseCost, validFrom: draft.validFrom, validTo: draft.validTo || null, notes: draft.notes || null };
