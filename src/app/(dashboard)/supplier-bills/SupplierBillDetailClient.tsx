...
export default function SupplierBillDetailClient({
  ...
  return (
    <div dir={locale === "ar" ? "rtl" : "ltr"} className="mx-auto flex w-full max-w-6xl min-w-0 flex-col gap-5 pb-12" data-supplier-bill-detail="workspace">
      ...
      {approved && bill.paymentSummary && <section className="rounded-xl border border-surface-variant bg-surface-container-lowest p-4" aria-labelledby="supplier-bill-payment-summary"><h2 id="supplier-bill-payment-summary" className="text-[14px] font-semibold text-primary">{dictionary.fields.paymentHistory}</h2><dl className="mt-4 grid grid-cols-2 gap-4 md:grid-cols-5"><div><dt className="text-[11px] font-semibold text-on-surface-variant">{dictionary.fields.payable}</dt><dd className="mt-1 font-semibold"><bdi dir="ltr">{formatSarAmount(locale, bill.paymentSummary.payable_amount)}</bdi></dd></div><div><dt className="text-[11px] font-semibold text-on-surface-variant">{dictionary.fields.advanceAllocated}</dt><dd className="mt-1 font-semibold"><bdi dir="ltr">{formatSarAmount(locale, bill.paymentSummary.advance_allocated_amount)}</bdi></dd></div><div><dt className="text-[11px] font-semibold text-on-surface-variant">{dictionary.fields.paid}</dt><dd className="mt-1 font-semibold"><bdi dir="ltr">{formatSarAmount(locale, bill.paymentSummary.paid_amount)}</bdi></dd></div><div><dt className="text-[11px] font-semibold text-on-surface-variant">{dictionary.fields.outstanding}</dt><dd className="mt-1 font-semibold"><bdi dir="ltr">{formatSarAmount(locale, bill.paymentSummary.outstanding_amount)}</bdi></dd></div><div><dt className="text-[11px] font-semibold text-on-surface-variant">{dictionary.fields.paymentStatus}</dt><dd className="mt-1 font-medium">{dictionary.paymentStatuses[bill.paymentSummary.payment_status]}</dd></div></dl>{bill.advanceAllocationHistory.length > 0 && <div className="mt-4"><h3 className="text-[12px] font-semibold text-on-surface-variant">{dictionary.fields.advanceAllocationHistory}</h3><ul className="mt-2 divide-y divide-surface-variant rounded-lg border border-surface-variant">{bill.advanceAllocationHistory.map((allocation) => <li key={allocation.id} className="flex flex-wrap items-center justify-between gap-3 px-3 py-3 text-[12px]"><span className="flex items-center gap-3"><PendingLink href={`/supplier-advances/${allocation.advance_id}`} pendingLabel={dictionary.fields.advanceAllocationHistory} className="font-semibold text-primary hover:underline"><bdi dir="ltr">{allocation.advance_number}</bdi></PendingLink><UiDateTimeText locale={locale} value={allocation.allocated_at} /></span><span className="flex items-center gap-3"><bdi dir="ltr" className="font-semibold">{formatSarAmount(locale, allocation.amount)}</bdi><span>{dictionary.advanceAllocationStatuses[allocation.status]}</span></span></li>)}</ul></div>}{bill.paymentHistory.length > 0 && <div className="mt-4"><h3 className="text-[12px] font-semibold text-on-surface-variant">{dictionary.fields.paymentHistory}</h3><ul className="mt-2 divide-y divide-surface-variant rounded-lg border border-surface-variant">{bill.paymentHistory.map((payment) => <li key={payment.id} className="flex flex-wrap items-center justify-between gap-3 px-3 py-3 text-[12px]"><span className="flex min-w-0 items-center gap-3"><bdi dir="ltr" className="font-semibold">{payment.payment_number}</bdi><UiDateText locale={locale} value={payment.payment_date} /><span>{dictionary.paymentMethods[payment.method]}</span></span><span className="flex items-center gap-3"><bdi dir="ltr" className="font-semibold">{formatSarAmount(locale, payment.amount)}</bdi><span>{dictionary.paymentStatuses[payment.status]}</span></span></li>)}</ul></div>}</section>}
