...
import type { SupplierAllocation, SupplierAllocationRow, SupplierAllocationStatus } from "./types";

function toSupplierAllocationRow(row: any): SupplierAllocationRow {
  return {
    ...row,
    status: row.status as SupplierAllocationStatus,
...
const SUPPLIER_ALLOCATION_STALE_ERROR =
...
export async function transitionSupplierAllocationStatus(
  ...
  try {
    ...
    const { data: updatedRow, error: updateError } = await supabase
      ...
      .eq("id", trimmedId)
      .eq("status", existingAllocation.status)
      .eq("is_deleted", false)
      .select()
      .maybeSingle();
    ...
    if (!updatedRow) {
      return { success: false, error: SUPPLIER_ALLOCATION_STALE_ERROR };
    }

    const canReadCost = await checkPermission("supplier_allocations:read_cost");
    const mappedData = mapSupplierAllocationRow(toSupplierAllocationRow(updatedRow), { canReadCost });
...
export async function createSupplierAllocation(
  ...
  try {
    ...
    let insertPayload: Database["public"]["Tables"]["service_supplier_allocations"]["Insert"];
    ...
    const canReadCost = await checkPermission("supplier_allocations:read_cost");
    const mappedData = mapSupplierAllocationRow(toSupplierAllocationRow(createdRow), { canReadCost });
...
export async function deleteSupplierAllocation(id: string): Promise<ActionResult<SupplierAllocation>> {
  try {
    ...
    if (existingAllocation.cost_source === "rate_card") {
      return { success: false, error: "Rate-card allocations cannot be deleted." };
    }
...
export async function restoreSupplierAllocation(id: string): Promise<ActionResult<SupplierAllocation>> {
  try {
    ...
    if (existingAllocation.cost_source === "rate_card") {
      return { success: false, error: "Rate-card allocations cannot be restored." };
    }
    ...
    const { data: rawSupplier, error: supplierError } = await supabase
      .from("suppliers")
    ...
    if (
      ...
      return {
        success: false,
        error: "Supplier is unavailable for allocation restoration.",
      };
    }
