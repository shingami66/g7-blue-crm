...
function toQuotationDetailRow(row: Record<string, unknown>): QuotationDetailRow {
  ...
  return {
    ...
    status: (typeof row.status === "string" ? row.status : "draft") as QuotationDetailRow["status"],
    ...
    snapshot_seller: (row.snapshot_seller as QuotationDetailRow["snapshot_seller"]) ?? null,
    snapshot_buyer: (row.snapshot_buyer as QuotationDetailRow["snapshot_buyer"]) ?? null,
    customers: (row.customers as QuotationDetailRow["customers"]) ?? null,
    services: (row.services as QuotationDetailRow["services"]) ?? null,
