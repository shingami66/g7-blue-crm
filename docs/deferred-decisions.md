...
- **Program classification:** The entries below are retained return-later items within the ACTIVE SaaS Performance & Scale Program. They are not a decision to defer known current-core performance/scale problems; P0 Customer360 closeout, P1 shared record-navigation closeout, P2 detail-page responsiveness architecture, P3 data-access round-trip reduction, P4 related-record UX bounding, and P5 narrow read models / projections are complete and published. P2 is published at `b46b9490bf7f739743c47d008c860f892cb49ca3` (`perf(detail): decouple secondary work from primary detail content`) with the accepted conclusion **PRIMARY DETAIL CONTENT SUCCESSFULLY SEPARATED FROM MATERIAL SECONDARY WORK**. P6 **SCALE-SAFE FINANCIAL READ MODELS** is the active phase in `project-roadmap.md` and `project-status.md` under `G7-SAAS-PERFORMANCE-P6-SCALE-SAFE-FINANCIAL-READ-MODELS-01`.
...
- **P5 closeout / active P6 task:** P5 is **COMPLETE / PUBLISHED / VERIFIED**. Service Detail → Related Quotations is published in `3b0ec95cec0b912f84fb4a2f0cdb9578c45a92df` (`perf(service): narrow related quotation projection`), replacing a full quotation row plus customer/service relations with a six-field route-specific projection while preserving one request. Customer360 secondary Services is published in `687371959bb75b0bb80394941c28c86ef478a1c8` (`perf(customer): narrow Customer360 service projection`), reducing `9 → 6` selected fields while preserving one request, filters, ordering, cardinality, operational facts, and financial reads. Both verdicts are **READ MODEL SUCCESSFULLY NARROWED — PERFORMANCE EFFECT NOT MATERIAL** and authoritative business/financial truth is preserved. **NO FURTHER MATERIAL P5 PROJECTION OPPORTUNITY JUSTIFIED.** The final classifications are Customer360 quotations **INSUFFICIENT MATERIAL BENEFIT**, Customer360 invoices **INSUFFICIENT MATERIAL BENEFIT / BUSINESS-FINANCIAL TRUTH RISK**, Customer360 payments **ALREADY APPROPRIATELY NARROW**, and shared Service/Invoice/Quotation details **SHARED CONTRACT MAKES NARROWING DISPROPORTIONATE / INSUFFICIENT EVIDENCE**. Do not create scalar-trim work or move generic shared-detail redesign into P6. P6 is **ACTIVE — SCALE-SAFE FINANCIAL READ MODELS**; P7-P10 remain **NOT STARTED / FUTURE PROGRAM PHASES**.
- **P6 scope guard:** P6 begins with evidence and architecture analysis of authoritative financial read scalability and does not pre-authorize SQL/RPC, migrations, indexes, caching, infrastructure, or implementation. **GENERIC SHARED SERVICE DETAIL REDESIGN IS NOT AUTOMATICALLY P6; GENERIC QUOTATION DETAIL REDESIGN IS NOT AUTOMATICALLY P6; GENERIC ENTITY-CONTRACT REDESIGN IS NOT AUTOMATICALLY P6; P6 OWNS SCALE-SAFE FINANCIAL READ MODEL WORK ONLY.** The return queue remains intact.
- **Invoice Detail attribution:** Current median is 2330 ms. Reopen for bounded actual-interaction latency attribution after the active P2 detail-page responsiveness work and any newly reranked higher-priority work are completed or reprioritized.
