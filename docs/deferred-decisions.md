...
## Vendors / Suppliers
- **Status:** Design direction, supplier directory DB foundation, live read-only supplier directory, create form, basic profile edit, optional fields persistence, dedicated blacklist workflow, internal read-only rate cards visibility, supplier allocations foundation, supplier allocations schemas/types/mappers/read-queries/actions/UI-panel, supplier allocations create UI, and manual supplier allocations edit UI resolved; supplier delete/restore, rate card write/finance workflows, and supplier allocations cancel/delete/restore UI remain deferred.
