...
## Vendors / Suppliers
- **Status:** Design direction, supplier directory DB foundation, live read-only supplier directory, create form, basic profile edit, optional fields persistence, dedicated blacklist workflow, internal read-only rate cards visibility, supplier allocations foundation, supplier allocations schemas/types/mappers/read-queries/actions/UI-panel, supplier allocations create UI, manual supplier allocations edit UI, manual supplier allocations cancel UI, and manual supplier allocations delete/restore UI resolved; supplier delete/restore and rate card write/finance workflows remain deferred.
