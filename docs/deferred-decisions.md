...
- **Next active product task (locked, exactly one):** `APPROVED-BILLING-SCOPE-MANAGEMENT-DESIGN-1`.
- **Responsive core P0:** source implementation complete (`RESPONSIVE_CORE_P0_IMPLEMENTED`). Durable flag **`RESPONSIVE_CORE_P0_MANUAL_SMOKE_PENDING`** — Team Lead deferred Mozfer browser/mobile smoke (`RESPONSIVE-CORE-P0-MOZFER-SMOKE-1`). Do **not** claim manual smoke PASS or formal responsive acceptance close until that smoke completes. Automated evidence: **107/107** PASS.
