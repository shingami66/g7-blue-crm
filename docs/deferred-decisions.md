...
- **Status:** Product policy resolved; management **design locked** (`docs/approved-billing-scope-management-design.md`); bounded DEV/DEMO runtime slices implemented; read-enrichment UI complete; create draft complete and pushed; draft edit/discard source implemented and manually accepted; remaining write CTAs incomplete; production authorization/apply remain deferred.
- **Implemented DEV/DEMO or completed history:** foundation schema and runtime contracts; draft creation and discard/edit actions (+ narrow discard/item-edit RPCs); line-safety review and approval **server actions**; invoice integration and billing ceiling; the Service Detail card with read-enrichment (`ABS-MGMT-UI-READ-ENRICH-1`: effective display state, source quotation, ceiling, invoiced/remaining when `invoices:read`, line safety, draft/history indicators, detail navigation); the nested read-only detail route; and the Admin/Manager **Create Draft** UI slice (`ABS-MGMT-UI-DRAFT-CREATE-1`) in current source, manually accepted in DEV/DEMO. Its commit/push remains a separate pending task.
- **Not implemented (current truth):** void action function; supersede action function; review/approve **write UI**. Server actions exist for create/edit/discard/review/approve, and the current management UI source now exposes Create Draft plus draft edit/discard. Schemas, permission keys, and `voided_*` / `superseded_*` columns do **not** equal working void/supersede actions. Status enum is `draft | approved | voided` only — **`superseded` is not a DB status** (timestamp/relationship only). Accountant masking of internal notes/reasons remains preserved.
- **Resolved V1 policy (intent):** Approved Billing Scopes are immutable after approval for ordinary edits. Uninvoiced scopes may later be voided or superseded once actions exist and financial behavior is locked. Invoiced scopes remain frozen for historical linkage; successor versions apply only to future invoices; existing invoices retain their original scope reference. Active ABS ceiling is authoritative when present; approved-quotation fallback when no active scope.
- **Durable flag — `ABS_VOID_SUPERSEDE_FINANCIAL_BEHAVIOR_PENDING`:** Before `ABS-MGMT-VOID-ACTION-1` or `ABS-MGMT-SUPERSEDE-ACTION-1`: define whether voiding the **active** scope blocks new invoicing, restores legacy approved-quotation fallback, or requires another explicit authority; define atomic supersede after partial invoicing; confirm migration/RPC need; ordinary approve must not silently supersede; one-active-scope invariant must remain enforceable. **Do not treat void/supersede as implementation-ready** until this flag is resolved. Do not solve inside ordinary UI slices 1–4.
- **Still deferred:** remaining management write UI (edit → review/approve); void/supersede actions (blocked on flag above); optional history list UI; production database authorization/apply; richer audit/history capabilities.
- **Locked implementation order:** `ABS-MGMT-UI-READ-ENRICH-1` (**complete**) → `ABS-MGMT-UI-DRAFT-CREATE-1` (**source-complete and manually accepted; pushed on main in `47d9a4f14f019e837224e6db6cababdab12a7610` and `7054cf34654266ca033c58c62f9dca6d94092967`) → `ABS-MGMT-UI-DRAFT-EDIT-1` (**complete; source implemented and manually accepted; current source remains uncommitted and unpushed until controlled commit/push**) → `ABS-MGMT-UI-REVIEW-APPROVE-1` (**active**) → void (**blocked**) → supersede (**blocked**); optional `ABS-MGMT-HISTORY-LIST-1`.
- **When to return:** Management UI slices follow the locked order; void/supersede only after the financial-behavior flag; production-authorization and audit/history as separate reviewed tasks.
...
- **Next active product task (locked, exactly one):** `ABS-MGMT-UI-REVIEW-APPROVE-1` (`ABS-MGMT-UI-DRAFT-CREATE-1` is source-complete and manually accepted; `ABS-MGMT-UI-DRAFT-EDIT-1` is source-complete and manually accepted; see `docs/approved-billing-scope-management-design.md` and project status).
- **ABS void/supersede:** durable flag **`ABS_VOID_SUPERSEDE_FINANCIAL_BEHAVIOR_PENDING`** remains open — slices `ABS-MGMT-VOID-ACTION-1` and `ABS-MGMT-SUPERSEDE-ACTION-1` remain blocked and not implementation-ready.
