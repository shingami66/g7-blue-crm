...
- **Implemented DEV/DEMO or completed history:** foundation schema and runtime contracts; draft creation and discard/edit actions (+ narrow discard/item-edit RPCs); line-safety review and approval **server actions**; invoice integration and billing ceiling; the Service Detail **read-only** card **with read-enrichment** (`ABS-MGMT-UI-READ-ENRICH-1`: effective display state, source quotation, ceiling, invoiced/remaining when `invoices:read`, line safety, draft/history indicators, detail navigation); and the nested **read-only** detail route.
- **Not implemented (current truth):** void action function; supersede action function; create/edit/review/approve/discard **write UI** (server actions exist for create/edit/discard/review/approve; UI CTAs pending starting with draft-create). Schemas, permission keys, and `voided_*` / `superseded_*` columns do **not** equal working void/supersede actions. Status enum is `draft | approved | voided` only — **`superseded` is not a DB status** (timestamp/relationship only). Accountant masking of internal notes/reasons remains preserved.
...
- **Still deferred:** remaining management write UI (create → edit → review/approve); void/supersede actions (blocked on flag above); optional history list UI; production database authorization/apply; richer audit/history capabilities.
- **Locked implementation order:** `ABS-MGMT-UI-READ-ENRICH-1` (**complete**) → `ABS-MGMT-UI-DRAFT-CREATE-1` (**active**) → draft edit → review/approve → void (**blocked**) → supersede (**blocked**); optional `ABS-MGMT-HISTORY-LIST-1`.
...
- **Smoke result:** `APPROVED-BILLING-SCOPE-DRAFT-CREATE-COMMIT-1` implemented the create-draft action in `4ec323f feat(billing): add approved scope draft creation` and manual DEV/DEMO smoke passed for quotation `9778cf05-ae13-4072-8d6d-0b2ec1e970fe`.
- **Smoke verification:** first create succeeded with `scopeId = 2fb8a324-4bd2-44be-8a23-a2b37e9b6e72`; duplicate second click returned `scope_duplicate_draft`.
...
- **Next active product task (locked, exactly one):** `ABS-MGMT-UI-DRAFT-CREATE-1` (`ABS-MGMT-UI-READ-ENRICH-1` complete — see `docs/approved-billing-scope-management-design.md` and project status).
