...
- **Supplier Allocations (Foundation, Schemas, & Read Queries Closed):** `SUPPLIER-ALLOCATIONS-FOUNDATION-1A` (database/permissions foundation), `SUPPLIER-ALLOCATIONS-SCHEMAS-1A` (domain types, Zod schemas, and mappers), and `SUPPLIER-ALLOCATIONS-READ-1A` (server-only read queries) are closed. `SUPPLIER-ALLOCATIONS-CREATE-MANUAL-1A` (server action for manual create) is closed. `SUPPLIER-ALLOCATIONS-CANCEL-1A` (server action for cancel) is closed. Other write actions, UI panels, Service Detail integration, and history UI remain pending/deferred.
