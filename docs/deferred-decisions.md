...
- **Program classification:** The entries below are retained return-later items within the ACTIVE SaaS Performance & Scale Program. They are not a decision to defer known current-core performance/scale problems; P0 Customer360 closeout and P1 shared record-navigation closeout are complete, and P2 detail-page responsiveness architecture is active in `project-roadmap.md` and `project-status.md`.
