...
- **Still deferred:** Supplier delete/restore, rate card create/edit/delete/restore workflows, rate card overlap enforcement in write flows, service supplier allocations runtime CRUD/actions/UI, supplier bookings/internal POs, supplier invoices/payments, supplier costing/margin/P&L reports, rate-card-driven quotation automation, any customer-facing exposure of supplier cost, Supplier PO PDF, WhatsApp/email, supplier portal, invoice attachments, supplier payment approval workflow, and any future supplier SQL/migrations beyond the completed directory foundation, rate cards foundation, and allocations foundation.
- **Supplier Allocations (Foundation & Schemas Closed):** `SUPPLIER-ALLOCATIONS-FOUNDATION-1A` (database/permissions foundation) and `SUPPLIER-ALLOCATIONS-SCHEMAS-1A` (domain types, Zod schemas, and mappers) are closed. Runtime implementation (CRUD actions, DB queries, UI panel, Service Detail integration, allocations history) remains pending/deferred.
  ...
  - Domain Module: `src/lib/supplier-allocations/` implements types, Zod validation schemas, and camelCase mappers with cost redaction logic based on `canReadCost`.
