...
- **Status:** Design direction approved; runtime decisions locked in `docs/approved-billing-scope-runtime-decisions.md`; implementation deferred.
- **Reason deferred:** Approved Billing Scope is the chosen future billing-authority model, and its foundation migration now exists in DEV/DEMO, but production-safe runtime, RBAC/RLS, invoice integration, and UI work still need separate reviewed implementation.
- **When to return:** `APPROVED-BILLING-SCOPE-LIVE-SCHEMA-ENFORCEABILITY-CHECK-1` and the follow-up sequence.
...
- **Follow-ups:** `APPROVED-BILLING-SCOPE-LIVE-SCHEMA-ENFORCEABILITY-CHECK-1`, `APPROVED-BILLING-SCOPE-MIGRATION-DRAFT-1`, `APPROVED-BILLING-SCOPE-RBAC-RLS-REVIEW-1`, `APPROVED-BILLING-SCOPE-INVOICE-INTEGRATION-DESIGN-1`, and `QUOTATION-REVISION-FALLBACK-DESIGN-1`.
