...
## Vendors / Suppliers
- **Status:** Design direction, supplier directory DB foundation, live read-only supplier directory, create form, basic profile edit, optional fields persistence, dedicated blacklist workflow, internal read-only rate cards visibility, supplier allocations foundation, supplier allocations schemas/types/mappers/read-queries/actions/UI-panel, and supplier allocations create UI resolved; supplier delete/restore, rate card write/finance workflows, and supplier allocations edit/cancel/delete/restore UI remain deferred.
...
- **Still deferred:** Supplier delete/restore, rate card create/edit/delete/restore workflows, rate card overlap enforcement in write flows, service supplier allocations write actions/UI (except manual create UI), supplier bookings/internal POs, supplier invoices/payments, supplier costing/margin/P&L reports, rate-card-driven quotation automation, any customer-facing exposure of supplier cost, Supplier PO PDF, WhatsApp/email, supplier portal, invoice attachments, supplier payment approval workflow, and any future supplier SQL/migrations beyond the completed directory foundation, rate cards foundation, and allocations foundation.
- **Supplier Allocations (Foundation, Schemas, Read Queries, Create, Cancel, Update, UI Panel 1A, & UI Create 1B Closed):** `SUPPLIER-ALLOCATIONS-FOUNDATION-1A` (database/permissions foundation), `SUPPLIER-ALLOCATIONS-SCHEMAS-1A` (domain types, Zod schemas, and mappers), `SUPPLIER-ALLOCATIONS-READ-1A` (server-only read queries), `SUPPLIER-ALLOCATIONS-CREATE-MANUAL-1A` (server action for manual create), `SUPPLIER-ALLOCATIONS-CANCEL-1A` (server action for cancel), `SUPPLIER-ALLOCATIONS-UPDATE-MANUAL-1A` (server action for manual update), `SUPPLIER-ALLOCATIONS-SERVICE-UI-PANEL-1A` (read-only service allocations panel), and `SUPPLIER-ALLOCATIONS-SERVICE-UI-CREATE-1B` (manual allocation create UI) are closed. Other write actions (delete/restore, rate-card allocation creation), edit/cancel/delete UI, and history UI remain pending/deferred.
