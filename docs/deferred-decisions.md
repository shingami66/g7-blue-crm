...
- **Program classification:** The entries below are retained return-later items within the ACTIVE SaaS Performance & Scale Program. They are not a decision to defer known current-core performance/scale problems; P0 Customer360 closeout, P1 shared record-navigation closeout, P2 detail-page responsiveness architecture, and P3 data-access round-trip reduction are complete and published. P2 is published at `b46b9490bf7f739743c47d008c860f892cb49ca3` (`perf(detail): decouple secondary work from primary detail content`) with the accepted conclusion **PRIMARY DETAIL CONTENT SUCCESSFULLY SEPARATED FROM MATERIAL SECONDARY WORK**. P4 related-record UX bounding is now the active phase in `project-roadmap.md` and `project-status.md`.
...
- **Active P4 task:** `G7-SAAS-PERFORMANCE-P4-RELATED-RECORD-UX-BOUNDING-01` is active under **RELATED-RECORD UX BOUNDING**. The Customer Detail related-invoice direction remains latest 10 contextual invoices plus View All to the dedicated Customer-filtered Invoice workspace, **PLANNED / DOCUMENTED / NOT IMPLEMENTED**. P5-P10 remain **NOT STARTED / FUTURE PROGRAM PHASES**.
