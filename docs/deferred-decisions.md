...
- **Next active product task (locked, exactly one):** `RESPONSIVE-CORE-P0-AUDIT-1` — audit-only evidence of page-level horizontal overflow / narrow-width usability on core CRM paths (shell, Dashboard, Customers, Services/Detail, Quotations, Invoices, Payments, Settings, Admin Users, core create/edit forms, blocking overlays). Classify page-level overflow vs table-local scroll vs long stored data vs PDF exclusions vs deferred supplier redesign. No responsive implementation until audit names source-proven defects. Following critical-path order (not active yet): `APPROVED-BILLING-SCOPE-MANAGEMENT-DESIGN-1` → financial lifecycle design/audit.
