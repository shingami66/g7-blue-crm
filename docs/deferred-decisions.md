...
- **Status:** Product policy resolved; management **design locked** (`docs/approved-billing-scope-management-design.md`); bounded DEV/DEMO runtime slices implemented; read-enrichment, create draft, and draft edit/discard are complete and pushed; Review/Approve is implemented and manually accepted in current uncommitted source; production authorization/apply remain deferred.
- **Implemented DEV/DEMO or completed history:** foundation schema and runtime contracts; draft creation and discard/edit actions (+ narrow discard/item-edit RPCs); line-safety review and approval server actions and current UI; invoice integration and billing ceiling; the Service Detail card with read-enrichment (`ABS-MGMT-UI-READ-ENRICH-1`); the nested detail route; and the Admin/Manager Create Draft, Draft Edit/Discard, and Review/Approve UI slices. Review/Approve remains uncommitted and unpushed pending controlled commit/push.
- **Not implemented (current truth):** void action function; supersede action function; Review/Approve commit/push only. Server actions and current UI exist for create/edit/discard/review/approve. Schemas, permission keys, and `voided_*` / `superseded_*` columns do **not** equal working void/supersede actions. Status enum is `draft | approved | voided` only — **`superseded` is not a DB status** (timestamp/relationship only). Accountant masking of internal notes/reasons remains preserved.
...
- **Still deferred:** Review/Approve controlled commit/push; void/supersede actions (blocked on flag above); optional history list UI; production database authorization/apply; richer audit/history capabilities.
- **Locked implementation order:** `ABS-MGMT-UI-READ-ENRICH-1` (**complete**) → `ABS-MGMT-UI-DRAFT-CREATE-1` (**complete and pushed**) → `ABS-MGMT-UI-DRAFT-EDIT-1` (**complete and pushed on main at `df7cf1e9ef9d5302162735bcc87a8aa567385073`**) → `ABS-MGMT-UI-REVIEW-APPROVE-1` (**complete in current source; commit/push pending**) → `ABS-MGMT-FINANCIAL-LIFECYCLE-DESIGN-1` (**next active implementation task**) → void (**blocked**) → supersede (**blocked**); optional `ABS-MGMT-HISTORY-LIST-1`.
...
- **Next active product task (locked, exactly one):** `ABS-MGMT-FINANCIAL-LIFECYCLE-DESIGN-1` (`ABS-MGMT-UI-DRAFT-CREATE-1`, `ABS-MGMT-UI-DRAFT-EDIT-1`, and `ABS-MGMT-UI-REVIEW-APPROVE-1` are source-complete and manually accepted; Review/Approve commit and push remain separate controlled actions).
