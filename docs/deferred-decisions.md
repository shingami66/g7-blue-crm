...
## Vendors / Suppliers
- **Status:** Design direction, supplier directory DB foundation, live read-only supplier directory, create form, basic profile edit, optional fields persistence, dedicated blacklist workflow, internal read-only rate cards visibility, supplier allocations foundation, supplier allocations schemas/types/mappers/read-queries/actions/UI-panel, supplier allocations create UI, manual supplier allocations edit UI, manual supplier allocations cancel UI, manual supplier allocations delete/restore UI, rate-card allocation creation, Supplier Booking DB numbering, Supplier Booking query layer, and narrow Supplier Booking actions resolved; supplier delete/restore and rate card edit/delete/restore workflows remain deferred.
...
- **Supplier Booking:** DB foundation, schemas, permissions, queries, DB numbering default, and narrow actions complete (`SUPPLIER-BOOKINGS-FOUNDATION-1`, `SUPPLIER-BOOKINGS-SCHEMAS-1A`, `SUPPLIER-BOOKINGS-PERMISSIONS-1A`, `SUPPLIER-BOOKINGS-QUERIES-1A`, `SUPPLIER-BOOKINGS-NUMBERING-DB-1`, `SUPPLIER-BOOKINGS-ACTIONS-1A`). Supplier Booking UI, customer-facing documents/messages/portal, supplier invoices, supplier payments, actual supplier costs, profit/margin reporting, and broader runtime behavior remain deferred. Terminology remains Supplier Booking / SBK.
