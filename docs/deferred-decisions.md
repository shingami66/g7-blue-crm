...
## Vendors / Suppliers
- **Status:** Supplier Directory V1 and Supplier Operations V1 are implemented. Supplier Operations provides internal, Service-scoped Allocation and Booking workflows: active Supplier lifecycle gates for Booking creation and Allocation restore, rate-card date validity, active-Booking Allocation mutation locks, conditional affected-row checks, accessible EN/AR cancellation dialogs, and distinct load-failure/empty states. Quantity uses `NUMERIC(10,3)` and unit cost uses `NUMERIC(14,2)`. The Supplier directory remains server-paginated at 10 records. Application-layer stale/dependency checks are nontransactional; production RLS/readiness is not claimed. Codex browser automation did not render the login DOM; Mozfer approved closeout without further browser smoke.
...
- **Historical read-only-slice verification:** Lint passed with only existing `<img>` warnings, `pnpm exec tsc --noEmit` passed, no bank or IBAN fields were selected/mapped/typed for UI/rendered, no `suppliers:write` usage was added, no create/edit/delete/restore behavior was added, and no SQL/schema/migration changes or supplier finance/future modules were introduced. That limited scope has since been superseded by Supplier Directory V1 above.
- **Still deferred:** Rate-card management edit/delete/restore and overlap enforcement, rate-card automation, allocation deduplication, cross-Service overlap/capacity, standalone/broader Supplier Booking routes/UI, Supplier Booking customer-facing documents/messages/portal, supplier invoices, payables, outbound payments, accounting workflows, actual-cost posting, supplier costing/margin/P&L reports, rate-card-driven quotation automation, any customer-facing exposure of supplier cost, invoice attachments, supplier payment approval workflow, Accountant Supplier-bank access, production RLS/readiness, and any future supplier SQL/migrations beyond the completed foundations. Current stale/dependency checks remain application-layer and nontransactional.
...
- **Historical server-action smoke verification:** first create succeeded with `scopeId = 2fb8a324-4bd2-44be-8a23-a2b37e9b6e72`; a duplicate request returned `scope_duplicate_draft`. This is not a manual UI double-click stress-test claim for `ABS-MGMT-UI-DRAFT-CREATE-1`.
- **Draft Discard Smoke:** `APPROVED-BILLING-SCOPE-DRAFT-DISCARD-SMOKE-DOCS-SYNC-1` verified the manual apply of draft discard migration `20260708110000_approved_billing_scope_draft_discard_function.sql` in DEV/DEMO and manual app smoke test (creating draft `0ace1c81-68c0-4cdd-8d9b-db563cd49949` and verifying its atomic deletion and item cleanup).
...
Accepted **non-blocking technical debt** notes from the DEV/DEMO browser acceptance (`PASS WITH WARN`) and later atomic create hardening. **Residuals 1–4 are closed for the live Deposit/Final create path:** `public.create_invoice_atomic` is installed in DEV/DEMO, **used by application create**, and DEV/DEMO runtime-smoke verified (`a83c1d28`; service_role only; no multi-query create write; no silent fallback). Canonical contract: [`docs/atomic-invoice-creation-contract.md`](./atomic-invoice-creation-contract.md). Migration: `supabase/migrations/20260717180000_atomic_invoice_create.sql` (`5ad23f25`). **Preserved caveats:** production apply/readiness is **not** claimed; `issueInvoiceAction` remains a separate update path; some RPC `error_code` values still map through safe generic UI fallback until dictionary expansion. No VAT/ZATCA/FATOORA claim.

**Task 20 DEV/DEMO runtime evidence (local application, DEV/DEMO data only, Admin):** Deposit creation PASS for `10,000.00 SAR` as `INV-2026-0032` with exactly one Draft Deposit and verified Service/Quotation linkage; Final creation PASS for server-derived `50,840.00 SAR` as `INV-2026-0033` with exactly one Draft Final and verified linkage; remaining billing authority reached `0.00 SAR`; Deposit and Final creation controls disappeared after full allocation; duplicate and fully allocated creation was safely prevented; no SQL, stack trace, constraint, or internal database details appeared in the UI; repository preflight/postflight matched byte-for-byte; no unexpected tracked or untracked files were created; production was not touched.

### 1. Legacy Quotation concurrent Deposit race (create path)
- **Status:** **Closed on live Deposit/Final create path** (DEV/DEMO RPC + app integration `a83c1d28` + runtime smoke PASS).
- **Current behavior:** `createInvoiceAction` calls `create_invoice_atomic` once; Service row is locked with `FOR UPDATE` before remaining recompute and insert.
- **Remaining caveat:** concurrent races on **other** invoice write surfaces (if any) outside this create RPC are not claimed closed; production apply not claimed.

### 2. Legacy Quotation ceiling branch (create path)
- **Status:** **Closed on live create path** — RPC enforces legacy Quotation ceiling only after exact-zero ABS history; active ABS ceiling otherwise; runtime smoke verified full allocation to remaining `0.00 SAR`.
- **Remaining caveat:** app billing-state UI/read resolvers still present authority modes for display; create financial authority is RPC-owned.

### 3. Lifecycle validation and Invoice insert atomicity (create path)
- **Status:** **Closed on live create path** — seven-state matrix re-check and insert run in one RPC transaction under Service lock.
- **Remaining caveat:** `issueInvoiceAction` (draft → sent) remains a separate sequential update path, not part of create atomicity.

### 4. Seven-state Deposit/Final create enforcement (create path)
- **Status:** **Closed on live create path** — Deposit/Final eligibility matrix enforced inside `create_invoice_atomic`; duplicate and fully allocated creation was safely prevented in DEV/DEMO runtime smoke.
- **Remaining caveat:** broader update-path triggers for all invoice mutations remain separate if ever needed; not a claim that every invoice write is RPC-gated.
...
- **Current behavior:** App requires positive proof of exact zero ABS history before legacy Quotation; Draft counts as history. Create RPC counts any ABS row as history (including Draft).
- **Risk classification:** Low–medium for non-create helpers if still divergent.
- **Mitigation today:** `invoiceHistoryProvesExactlyZero` / card `draft_only` no longer reopens legacy authority; create path uses RPC history count.
- **Future hardening:** Optional DB helper alignment for non-create surfaces if still divergent.

### 6. Deposit client maximum and remaining authority
- **Status:** **Addressed in application UX** (Wave A Task 14): Deposit client max/validation use remaining authority. **Server create path atomic race closed** via `create_invoice_atomic` (`a83c1d28`) and DEV/DEMO runtime smoke.
- **Residual:** browser above-remaining lane remains automated-evidence-only.
...
Supplier deferrals remain for supplier delete/restore, supplier bank/IBAN collection/editing, rate cards, allocations beyond closed slices, supplier invoices/payments, supplier portal, costing/margin/P&L, and payment approval workflows. Supplier Booking foundation and the narrow internal Service Detail UI are complete. Broader Supplier Booking capabilities remain deferred, including standalone routes, PDFs, WhatsApp/email, supplier portal, supplier invoices/payments, actual supplier costs, profit/margin reporting, edit/delete/restore, any status expansion beyond draft/cancelled, and any customer-facing supplier cost exposure. Full ZATCA/FATOORA Phase 2 remains deferred until official VAT registration and a separate approved design. Global Search may remain later until Sprint 1 workflow blockers are implemented.
...
- `Supplier Bookings` (standalone routes, PDFs, WhatsApp/email, supplier portal, supplier invoices/payments, actual supplier costs, profit/margin reporting, edit/delete/restore, status expansion, and broader runtime behavior)
...
- new dependencies
- rate-card automation
- `Supplier Bookings` expansions beyond the closed narrow Service Detail UI
...
- Feature 005 + P3 + **P5 visual acceptance** are **formally closed** for authenticated CRM UI (date/time/range/bidi, list alignment, Settings copy leakage, structured date components). Pushed acceptance commit: `aaf6563 fix(i18n): complete bilingual visual acceptance`. Closeout docs commit: `e731d4d docs(i18n): formally close Feature 005`.
- **Next active product task (locked, exactly one):** `ABS-MGMT-FINANCIAL-LIFECYCLE-DESIGN-COMMIT-1` (all prior ABS UI slices through Review/Approve are implemented and pushed).
- **ABS void/supersede:** **`ABS_VOID_SUPERSEDE_SERVICE_LIFETIME_CEILING_LOCKED`** resolves the prior pending flag. Implementation remains gated by `ABS-MGMT-FINANCIAL-LIFECYCLE-MIGRATION-PREFLIGHT-1`, reviewed SQL/migration, separate DEV/DEMO apply/verification, actions/tests, and UI/smoke tasks.
- **Responsive core P0:** source implementation complete (`RESPONSIVE_CORE_P0_IMPLEMENTED`) plus body-overflow remediation (`RESPONSIVE-CORE-P0-SMOKE-FIX-1`). Manual re-smoke: **PASS by Mozfer manual browser evidence** (`RESPONSIVE-CORE-P0-MOZFER-RE-SMOKE-1`). Flag **`RESPONSIVE_CORE_P0_MANUAL_SMOKE_PENDING` is closed** (no longer unresolved/active). Agent did **not** perform browser smoke. Automated evidence: **107/107** PASS.
- **Supplier mobile detail (`R-P0-02`):** deferred to full Supplier redesign (`DEFERRED_SUPPLIER_REDESIGN`); no temporary mobile drawer/route in the responsive implement slice.
...
- `document_locale` schema/runtime wiring (must not inherit creator UI session locale).
- Customer `preferred_language`.
- Hijri calendar.
- Booking terminology final commercial copy.
