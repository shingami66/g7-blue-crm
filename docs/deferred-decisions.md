...
- **Supplier Allocations (Foundation, Schemas, Read Queries, Create, Cancel, & Update Closed):** `SUPPLIER-ALLOCATIONS-FOUNDATION-1A` (database/permissions foundation), `SUPPLIER-ALLOCATIONS-SCHEMAS-1A` (domain types, Zod schemas, and mappers), `SUPPLIER-ALLOCATIONS-READ-1A` (server-only read queries), `SUPPLIER-ALLOCATIONS-CREATE-MANUAL-1A` (server action for manual create), `SUPPLIER-ALLOCATIONS-CANCEL-1A` (server action for cancel), and `SUPPLIER-ALLOCATIONS-UPDATE-MANUAL-1A` (server action for manual update) are closed. Other write actions (delete/restore, rate-card allocation creation), UI panels, Service Detail integration, and history UI remain pending/deferred.
