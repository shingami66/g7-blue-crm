...
## Vendors / Suppliers
- **Status:** Design direction, supplier directory DB foundation, live read-only supplier directory, create form, basic profile edit, optional fields persistence, dedicated blacklist workflow, internal read-only rate cards visibility, and supplier allocations foundation resolved; supplier delete/restore, rate card write/finance workflows, and supplier allocations runtime CRUD/actions/UI remain deferred.
...
- **Supplier Allocations (Foundation Closed):** `SUPPLIER-ALLOCATIONS-FOUNDATION-1A` is closed (implemented database and permissions foundation). Runtime implementation (CRUD, actions, UI, Service Detail integration, allocations history) remains pending/deferred.
