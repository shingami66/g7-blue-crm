...
- **Still deferred:** Supplier delete/restore, rate card create/edit/delete/restore workflows, rate card overlap enforcement in write flows, service supplier allocations, supplier bookings/internal POs, supplier invoices/payments, supplier costing/margin/P&L reports, rate-card-driven quotation automation, any customer-facing exposure of supplier cost, Supplier PO PDF, WhatsApp/email, supplier portal, invoice attachments, supplier payment approval workflow, and any future supplier SQL/migrations beyond the completed directory foundation and rate cards foundation.
- **Supplier Allocations (Design Approved):** `SUPPLIER-ALLOCATIONS-DESIGN-1` is complete and approved. Implementation remains deferred/pending. No table or migration exists yet. UI in Service detail only. Pre-quotation approval allowed. `approved_quotation_id` nullable (for reporting). `supplier_rate_card_id` optional. Manual cost allowed (`cost_source` = `rate_card` | `manual_estimate`). Statuses `draft` | `planned` | `selected` | `cancelled`. MVP Permissions: No `supplier_allocations:approve` permission (Not included in MVP). Operations/Sales/Viewer no access. Admin/Manager full access. Separating `supplier_allocations:read` and `supplier_allocations:read_cost` (Server-only access, no broad grants). Blacklist: Block new allocations for blacklisted/inactive suppliers. Cancelled Service: If parent Service is cancelled, block new allocations and keep existing allocations as read-only historical planning records. Category Tech Debt: `category` is free text for MVP; future settings-driven/dropdown categories should be evaluated for reporting consistency.
