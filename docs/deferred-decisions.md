...
## Vendors / Suppliers
- **Status:** Design direction, supplier directory DB foundation, live read-only supplier directory, create form, basic profile edit, optional fields persistence, dedicated blacklist workflow, internal read-only rate cards visibility, supplier allocations foundation, supplier allocations schemas/types/mappers/read-queries/actions/UI-panel, supplier allocations create UI, manual supplier allocations edit UI, manual supplier allocations cancel UI, manual supplier allocations delete/restore UI, rate-card allocation creation, Supplier Booking DB numbering, Supplier Booking query layer, narrow Supplier Booking actions, and narrow internal Supplier Booking Service Detail UI resolved; supplier delete/restore and rate card edit/delete/restore workflows remain deferred.
