...
- **Program classification:** The entries below are retained return-later items within the ACTIVE SaaS Performance & Scale Program. They are not a decision to defer known current-core performance/scale problems; P0 Customer360 closeout, P1 shared record-navigation closeout, P2 detail-page responsiveness architecture, P3 data-access round-trip reduction, and P4 related-record UX bounding are complete and published. P2 is published at `b46b9490bf7f739743c47d008c860f892cb49ca3` (`perf(detail): decouple secondary work from primary detail content`) with the accepted conclusion **PRIMARY DETAIL CONTENT SUCCESSFULLY SEPARATED FROM MATERIAL SECONDARY WORK**. P5 narrow read models / projections is now the active phase in `project-roadmap.md` and `project-status.md`.
...
- **Active P5 task:** `G7-SAAS-PERFORMANCE-P5-NARROW-READ-MODELS-AND-PROJECTIONS-01` is active under **NARROW READ MODELS / PROJECTIONS**. Begin with field-use verification for material projections and broad joins, followed by focused tests and independent review. P6-P10 remain **NOT STARTED / FUTURE PROGRAM PHASES**.
