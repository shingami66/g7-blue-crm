...
## Vendors / Suppliers
- **Status:** Supplier Directory V1 is implemented and user-owned runtime-smoke verified in local DEV/DEMO: responsive detail, safe directory-summary reads, required create/edit and VAT-pairing validation, lifecycle and blacklist workflows, Admin-only bank reads/writes, and Admin-only soft delete/restore. The Supplier directory is server-paginated at 10 records, and the legacy static dataset, UUID fallback, and `recent_project` remnants are removed. The delete dependency check for active allocations/bookings is application-layer and nontransactional; production RLS/readiness is not claimed. Historical supplier directory, rate-card, allocation, and Supplier Booking foundations remain resolved.
...
- **Still deferred:** Rate card edit/delete/restore workflows, rate card overlap enforcement in write flows, service supplier allocations write actions/UI (except manual create, edit, cancel, and delete/restore UI), standalone/broader Supplier Booking routes/UI, Supplier Booking customer-facing documents/messages/portal, supplier invoices, payables, outbound payments, accounting workflows, actual supplier costs, supplier costing/margin/P&L reports, rate-card-driven quotation automation, any customer-facing exposure of supplier cost, invoice attachments, supplier payment approval workflow, Accountant Supplier-bank access, production RLS/readiness, and any future supplier SQL/migrations beyond the completed directory foundation, rate cards foundation, allocations foundation, and Supplier Booking numbering default.
