...
- **Status:** Product policy resolved; management **design locked** (`docs/approved-billing-scope-management-design.md`); bounded DEV/DEMO runtime slices implemented; user-facing management UI incomplete; production authorization/apply remain deferred.
- **Implemented DEV/DEMO or completed history:** foundation schema and runtime contracts; draft creation and discard/edit actions (+ narrow discard/item-edit RPCs); line-safety review and approval **server actions**; invoice integration and billing ceiling; the Service Detail **read-only** card; and the nested **read-only** detail route.
- **Not implemented (current truth):** void action function; supersede action function; full write UI (create/edit/review/approve/discard CTAs). Schemas, permission keys, and `voided_*` / `superseded_*` columns do **not** equal working actions. Status enum is `draft | approved | voided` only — **`superseded` is not a DB status** (timestamp/relationship only).
- **Resolved V1 policy (intent):** Approved Billing Scopes are immutable after approval for ordinary edits. Uninvoiced scopes may later be voided or superseded once actions exist and financial behavior is locked. Invoiced scopes remain frozen for historical linkage; successor versions apply only to future invoices; existing invoices retain their original scope reference.
...
- **Still deferred:** full management write UI (ordered slices after design); void/supersede actions (blocked on flag above); optional history list UI; production database authorization/apply; richer audit/history capabilities.
- **Locked implementation order:** `ABS-MGMT-UI-READ-ENRICH-1` (active) → draft create → draft edit → review/approve → void (**blocked**) → supersede (**blocked**); optional `ABS-MGMT-HISTORY-LIST-1`.
...
- **Next active product task (locked, exactly one):** `ABS-MGMT-UI-READ-ENRICH-1` (ABS management design complete — see `docs/approved-billing-scope-management-design.md`).
- **ABS void/supersede:** durable flag **`ABS_VOID_SUPERSEDE_FINANCIAL_BEHAVIOR_PENDING`** — slices `ABS-MGMT-VOID-ACTION-1` and `ABS-MGMT-SUPERSEDE-ACTION-1` remain blocked and not implementation-ready.
- **Responsive core P0:** source implementation complete (`RESPONSIVE_CORE_P0_IMPLEMENTED`). Durable flag **`RESPONSIVE_CORE_P0_MANUAL_SMOKE_PENDING`** — Team Lead deferred Mozfer browser/mobile smoke (`RESPONSIVE-CORE-P0-MOZFER-SMOKE-1`). Do **not** claim manual smoke PASS or formal responsive acceptance close until that smoke completes. **Do not reopen or mark complete** in ABS work. Automated evidence: **107/107** PASS.
