...
- **Status:** Product policy resolved; management **design locked** (`docs/approved-billing-scope-management-design.md`); bounded DEV/DEMO runtime slices implemented; **read-enrichment UI complete** (source; commit/push pending); remaining write CTAs incomplete; production authorization/apply remain deferred.
...
- **Not implemented (current truth):** void action function; supersede action function; draft edit/discard/review/approve **write UI**. Server actions exist for create/edit/discard/review/approve, but only Create Draft is exposed by the current management UI source. Schemas, permission keys, and `voided_*` / `superseded_*` columns do **not** equal working void/supersede actions. Status enum is `draft | approved | voided` only — **`superseded` is not a DB status** (timestamp/relationship only). Accountant masking of internal notes/reasons remains preserved.
...
- **Locked implementation order:** `ABS-MGMT-UI-READ-ENRICH-1` (**complete**) → `ABS-MGMT-UI-DRAFT-CREATE-1` (**source-complete and manually accepted; commit/push pending separately**) → `ABS-MGMT-UI-DRAFT-EDIT-1` (**active**) → review/approve → void (**blocked**) → supersede (**blocked**); optional `ABS-MGMT-HISTORY-LIST-1`.
...
- **Next active product task (locked, exactly one):** `ABS-MGMT-UI-DRAFT-EDIT-1` (`ABS-MGMT-UI-DRAFT-CREATE-1` is source-complete and manually accepted; its commit/push is a separate pending task — see `docs/approved-billing-scope-management-design.md` and project status).
