...
- **Program classification:** The entries below are retained return-later items within the ACTIVE SaaS Performance & Scale Program. They are not a decision to defer known current-core performance/scale problems; P0 Customer360 closeout, P1 shared record-navigation closeout, and P2 detail-page responsiveness architecture are complete and published. P2 is published at `b46b9490bf7f739743c47d008c860f892cb49ca3` (`perf(detail): decouple secondary work from primary detail content`) with the accepted conclusion **PRIMARY DETAIL CONTENT SUCCESSFULLY SEPARATED FROM MATERIAL SECONDARY WORK**. P3 data-access round-trip reduction is active in `project-roadmap.md` and `project-status.md`.
...
- **Active P3 task:** `G7-SAAS-PERFORMANCE-P3-DATA-ACCESS-ROUND-TRIP-REDUCTION-01` is active. Existing evidence supports **PARTLY — MATERIAL IN SPECIFIC PATHS** for remote Data API request multiplication and unnecessary round trips; it does not establish PostgreSQL engine slowness or authorize indexes, SQL/RPC redesign, caching, infrastructure/region changes, or framework/data-platform replacement. P4-P10 remain **NOT STARTED / FUTURE PROGRAM PHASES**.
