...
**Application-enforced (pushed source; not full DB guarantees):** five billing-authority modes (`active_abs`, `historical_abs_only`, `legacy_quotation`, `no_authority`, `unavailable`); legacy zero-history Quotation fallback qualification; full seven-state Deposit/Final lifecycle matrix; control visibility; remaining/ceiling checks for Deposit/Final create; safe UI error presentation; authoritative money parsing distinguishing zero from unavailable. Deposit/Final create remains multi-query application orchestration until the atomic RPC is implemented, applied, and integrated.

**Atomic Invoice create RPC (designed, not installed):** Canonical contract locked in [`docs/atomic-invoice-creation-contract.md`](./atomic-invoice-creation-contract.md) for proposed `public.create_invoice_atomic(...)`. That document is **design-only** — the function is **not** implemented and **not** installed in DEV/DEMO or production. Residuals 1–4 (legacy Quotation race/ceiling, non-atomic lifecycle+insert, incomplete atomic seven-state create enforcement) remain open until migration, Mozfer DEV/DEMO apply verification, and application integration complete.

**Deferred database hardening (not installed):** atomic Invoice create RPC (contract locked; implementation pending); legacy Quotation ceiling branch inside create path; atomic lifecycle validation plus insert; complete seven-state Deposit/Final create enforcement inside that RPC; optional broader write-side numeric work beyond current ABS write normalization.
