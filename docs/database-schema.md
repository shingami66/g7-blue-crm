...
- Services app UI/routes/server actions are implemented for list/create/detail/edit plus explicit guarded Start Execution, Complete Service, and reasoned cancellation; ordinary edit remains limited to Inquiry/Quoted and cannot change status directly.
...
- **Lifecycle implementation status:** Forward-only migrations `20260803090000_quotation_approval_internal_abs_activation.sql`, `20260803100000_quotation_approval_service_status_transition_fix.sql`, `20260803110000_service_execution_lifecycle_actions.sql`, `20260803120000_service_cancellation_guarded_action.sql`, and `20260803130000_deposit_service_audit_and_completed_billing_fix.sql` provide the delivered quotation-to-internal-ABS activation, Service execution/cancellation, Deposit audit, and Completed Final billing contracts. DEV/DEMO application and verification are owner-confirmed for the delivered milestone; production is not claimed. The application action and secondary technical-surface UI for ABS Void are delivered; Supersede management UI remains unshipped. Management design: `docs/approved-billing-scope-management-design.md`.
...
- Still deferred or not authorized: production database authorization/apply, Supersede management UI, invoice/payment correction or accounting treatment, and richer dedicated ABS history beyond the delivered evidence-based Service Activity History.
