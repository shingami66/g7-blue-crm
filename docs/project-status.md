...
- **Current verified repository baseline (23 August 2026):** `main`, local HEAD, and `origin/main` are synchronized at `b46b9490bf7f739743c47d008c860f892cb49ca3` (`0 ahead / 0 behind`). Unrelated local state is preserved. This HEAD publishes the completed P2 detail-page responsiveness architecture.
...
- **P3 — ACTIVE:** Program phase **DATA-ACCESS ROUND-TRIP REDUCTION**; exact active engineering task: `G7-SAAS-PERFORMANCE-P3-DATA-ACCESS-ROUND-TRIP-REDUCTION-01`. Existing evidence establishes remote Data API request multiplication and unnecessary round trips as material in specific paths; this does not establish PostgreSQL engine slowness or authorize SQL/RPC redesign, indexes, caching, infrastructure/region changes, or framework/data-platform replacement.
- **Program state:** P0 is **COMPLETE**; P1 is **COMPLETE / PUBLISHED / VERIFIED**; P2 is **COMPLETE / PUBLISHED / VERIFIED**; P3 is **ACTIVE**; P4-P10 are **NOT STARTED / FUTURE PROGRAM PHASES** under the existing roadmap.
- **Customer Detail invoice UX direction (planned, not yet implemented):** show the latest 10 contextual invoices, with View All to the dedicated invoices workspace filtered to the Customer. Visible preview limits must never redefine authoritative financial summaries or financial activity.
...
- **Next-work boundary:** The active evidence-backed engineering priority is P3 data-access round-trip reduction (`G7-SAAS-PERFORMANCE-P3-DATA-ACCESS-ROUND-TRIP-REDUCTION-01`). Supersede, richer financial correction, Change Orders, and accounting expansion remain separately deferred.
