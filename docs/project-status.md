...
- **Current verified repository baseline (22 August 2026):** `main`, local HEAD, and `origin/main` are synchronized at `2bfa2fa1155813c97162a2498a98d1a0b2cd31e6` (`0 ahead / 0 behind`). Unrelated local state is preserved. The validated but unpublished Customer360 invoice-loading optimization is a separate working-state candidate, not published baseline behavior.
...
- **Current P0 — close the validated Customer360 slice:** Before a new performance implementation task starts, add the regression proof that a live invoice outside the bounded preview still contributes to financial activity, complete the required review, and publish the bounded-preview/complete-facts optimization. The validated candidate reduced Customer Detail actual-interaction median from approximately 3580 ms to approximately 2261.5 ms while preserving complete financial truth.
- **Next shared candidate after P0:** `G7-SHARED-RECORD-NAVIGATION-CRITICAL-PATH-REMOVAL-01` is the leading bounded shared performance candidate. Its exact mechanism remains evidence-led; no specific implementation is pre-authorized by this status record.
- **Customer Detail invoice UX direction:** show the latest 10 contextual invoices, with View All to the dedicated invoices workspace filtered to the Customer. Visible preview limits must never redefine authoritative financial summaries or financial activity.
...
- **Next-work boundary:** The active evidence-backed engineering priority is Customer Detail latency attribution (`G7-CUSTOMER-DETAIL-ACTUAL-INTERACTION-LATENCY-ATTRIBUTION-01`). Supersede, richer financial correction, Change Orders, and accounting expansion remain separately deferred.
...
- **Workspace Location and Governance Rules:**
  ...
  - Canonical branch: `main`; current published checkpoint: `283cd5808845a846bb5c7f477a21f4eb3fc9700c`. The former `93d3f132b3ff756f4c49904da8622e40babdaa18` documentation-rebaseline authority and `90adf8f` discovery baseline remain historical evidence only.
