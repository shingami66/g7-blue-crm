...
- **Feature 008 invoices eligible-Service chooser:** The planning packet exists and is active through `.specify/feature.json` at `specs/008-invoices-eligible-service-chooser`. The approved product direction is one global `Create Invoice` CTA, followed by Deposit/Final type selection and a type-specific eligible-Service selector that navigates back to existing Service Detail billing authority. Runtime implementation has not started and remains separately unauthorized until planning review passes.
- **Feature 008 planning review:** The final independent Spec Kit review passed. Planning findings are fully remediated and the packet is implementation-ready from a planning and authority-contract perspective. Runtime implementation has not started and remains separately owner-authorized.
...
- **Product sequence:** Feature 007 is complete and historical. Feature 008 planning is complete and approved for the eligible-Service chooser in Invoices. The next controlled phase is runtime implementation, subject to separate owner authorization. Neither entry point may create a standalone document or add a second mutation authority.
