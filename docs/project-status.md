...
- **Feature 007 Quotations eligible-Service selector:** Active planning packet only. The global Quotations entry will replace its indirect `/services` navigation with a selector for eligible Services, then deep-link only to `/quotations/new?serviceId=<selected-service-id>`. Runtime implementation has not started.
- **Feature 007 authority boundary:** The selector remains Service-scoped and read/navigation-only. It must not create a Quotation, accept an independent customer, submit financial values, duplicate `createQuotation` or `create_quotation_with_items`, or introduce a second mutation authority. The active planning packet is `specs/007-quotations-eligible-service-selector`.
...
- **Product sequence:** Feature 007 plans the eligible-Service selector in Quotations. The eligible-Service chooser in Invoices remains separate and deferred until Feature 007 is complete. Neither entry point may create a standalone document or add a second mutation authority.
...
- **Next safe product/engineering direction:** Feature 007 Quotations eligible-Service selector is active planning only. Its runtime implementation, review, Mozfer-owned browser smoke, commit, and push remain separate; the Invoices Service chooser remains deferred until Feature 007 is complete. ABS history/read polish, Void UI, successor UI, and role browser smoke remain deferred.
