...
- **Current published checkpoint (22 August 2026):** `main`, local HEAD, and `origin/main` are synchronized at `283cd5808845a846bb5c7f477a21f4eb3fc9700c` (`0 ahead / 0 behind`). The published baseline independently passed frozen dependency preparation, focused tests, TypeScript, targeted lint, and the production build.
- **Published performance/dependency closures:** Service Detail now performs independent post-service reads concurrently after their actual prerequisites; `services:read`, `services:read_billing_summary`, and `invoices:read` remain distinct reads. The narrowly scoped Billing Summary authority contract is published with only `billingCeiling`, `activePriorInvoiceTotal`, and `remainingUninvoicedAmount`; it does not restore Full Billing State to the Service Detail parent graph. The published Payments compatibility closure contains only the minimal ModuleSearch `onClear` contract.
- **Current sufficiently measured critical-journey ranking:** Customer Detail 3580 ms median; Service Detail 2995 ms; Invoice Detail 2330 ms; Supplier workflow 1890 ms; Payments 317 ms. This is a corrected, actual-interaction-to-stable-destination measurement ranking, not a claim that every application journey has been ranked.
- **Current performance priority and exact next engineering task:** Customer Detail latency attribution — `G7-CUSTOMER-DETAIL-ACTUAL-INTERACTION-LATENCY-ATTRIBUTION-01`. G4 remains open and proceeds iteratively: measure -> attribute -> repair -> verify -> rerank.
