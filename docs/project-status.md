...
## 1.3 Current Review and Remediation Checkpoint (11 August 2026)

- **Current phase:** `G3 CLOSEOUT / G4 NEXT GATE — BOUNDED READ PATHS AND SCALE`.
...
- **Remediation program:** 12 Goals, G1 through G12. G1 is closed and pushed (`e34ea417...`); G2 Payment Precision (`W2-PAY-003`) is closed and pushed at `db68492`. Remote baseline is `7b0bb0b5adeb047e6905f5091071a445b1f91faa`. G3 Reporting truth and period semantics implementation remediation is progressed: G3-A1 (Reports Center Reporting Truth, local checkpoint `6bdc87b222488bcf4be04b4fb19ccac3574c67d7`) and G3-B1 (Customer 360 Financial Truth & Parity, local checkpoint `cbc39f0`) are completed and verified clean locally.
- **Repository baseline & Git state:** `D:/G7/g7-crm` on branch `main`. Remote baseline `origin/main` is `7b0bb0b5adeb047e6905f5091071a445b1f91faa`. Local campaign is 2 commits ahead of origin (`6bdc87b` and `cbc39f0`); no push authority is claimed.
...
- **Next Controlled Campaign Gate:** `G4: Bounded read paths and scale` (mapped to `M-02`, `M-04`, `W3-PERF-005`, `W3-QUERY-001`, `W3-SCALE-003/004`), which requires measurement-first evidence and Gemini discovery before implementation.
