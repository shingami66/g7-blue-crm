...
| P4 | ACTIVE — Related-record UX bounding | Exact active engineering task: `G7-SAAS-PERFORMANCE-P4-RELATED-RECORD-UX-BOUNDING-01`. Customer Detail target is latest 10 contextual invoices plus View All to full Customer-filtered invoice history. This remains planned/documented and is not implemented. Preview rows never define financial truth. |
...
**P4 activation:** P4 is now the active **RELATED-RECORD UX BOUNDING** phase under `G7-SAAS-PERFORMANCE-P4-RELATED-RECORD-UX-BOUNDING-01`. It targets evidence-backed bounded related-record presentation with explicit access to complete workspaces where appropriate. The Customer Detail invoice direction is latest 10 contextual invoices plus View All to the dedicated Customer-filtered Invoice workspace; it remains planned/documented and not implemented. Full financial summary and activity remain complete and authoritative independent of preview size.
...
- **G4 current execution state:** The system-wide diagnostic is complete enough to activate the SaaS Performance & Scale Program above. P0 Customer360 closure, P1 shared record-navigation closeout, P2 detail-page responsiveness architecture, and P3 data-access round-trip reduction are complete: the bounded-preview/complete-facts slice, >preview-boundary financial-activity regression proof, P1 four-family runtime verification, P2 primary/secondary runtime recertification, P3 duplicate-read reduction, reviews, publications, and validation cleanup are closed. P4 related-record UX bounding is now active. Service Detail's published independent-read concurrency repair and the retained Service/Invoice/Supplier/Payments/J4/J6 return-later queue remain discoverable in `deferred-decisions.md`. G4 proceeds measure -> evidence-backed implementation -> verify -> rerank.
...
- Performance/Scale: G4 remains open with completed P0, P1, P2, and P3 publications; remaining work proceeds by evidence-backed measure -> attribute -> repair -> verify -> rerank, beginning with active P4 related-record UX bounding.
...
### Current SaaS Performance & Scale Checkpoint — `0aed85835501c24d7834401d8cc9f296cc9de5cf`

- `main`, local HEAD, and `origin/main` are synchronized at this verified baseline (`0 ahead / 0 behind`). The P3 data-access round-trip reduction is published at this HEAD; P0, P1, P2, and P3 are complete, and P4 is active.
...
- **Immediate current work — P4 ACTIVE:** execute the evidence-led `G7-SAAS-PERFORMANCE-P4-RELATED-RECORD-UX-BOUNDING-01` objective. Bound related-record presentation route by route, beginning with the Customer latest-10 contextual invoice preview plus View All, without redefining financial truth or presuming database, caching, infrastructure, or platform changes.
...
- **Delivered product state / current engineering priority:** Feature 007, Feature 008, the latest Quotation/Invoice UX batch, quotation/internal-ABS activation, ABS Void, the Service lifecycle milestone, and P2 detail-page responsiveness architecture are delivered. P0 Customer360 invoice-loading closeout and P1 shared record-navigation closeout are complete; the current evidence-backed engineering priority is P3 data-access round-trip reduction (`G7-SAAS-PERFORMANCE-P3-DATA-ACCESS-ROUND-TRIP-REDUCTION-01`). Feature 009 remains inactive, and Supersede remains deferred.
- **Workspace Location and Governance Rules:**
  ...
  - Canonical branch: `main`; current published checkpoint: `b46b9490bf7f739743c47d008c860f892cb49ca3` (local HEAD and `origin/main`, `0 ahead / 0 behind`). The `90adf8faa33c4af1c0049b53817f5f95896a761f` frozen discovery baseline remains historical evidence only.
...
- **Next safe product/engineering direction:** P3 data-access round-trip reduction (`G7-SAAS-PERFORMANCE-P3-DATA-ACCESS-ROUND-TRIP-REDUCTION-01`) is the active evidence-backed engineering priority. Reports Center remains a separate V1 P1 item and is not the active SaaS Performance & Scale phase; Supersede remains a separate deferred revision workflow. Clerk invitation/webhook smoke remains deferred until production/UAT readiness.
