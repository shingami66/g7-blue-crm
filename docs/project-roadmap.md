...
| P6 | ACTIVE — Scale-safe financial read models | Exact active task: `G7-SAAS-PERFORMANCE-P6-SCALE-SAFE-FINANCIAL-READ-MODELS-01`. Begin with evidence and architecture analysis of authoritative financial-read scalability while preserving complete financial truth. No SQL/RPC, migration, index, cache, infrastructure, or redesign solution is pre-authorized. Generic Service/Quotation detail and entity-contract redesign are not automatically P6. |
...
Program state: **P0 COMPLETE / PUBLISHED / VALIDATED**, **P1 COMPLETE / PUBLISHED / VERIFIED**, **P2 COMPLETE / PUBLISHED / VERIFIED**, **P3 COMPLETE / PUBLISHED / VERIFIED**, **P4 COMPLETE / PUBLISHED / VERIFIED**, **P5 COMPLETE / PUBLISHED / VERIFIED**, **P6 ACTIVE — SCALE-SAFE FINANCIAL READ MODELS**, and **P7-P10 NOT STARTED / FUTURE PROGRAM PHASES**. P1 removed shared record-navigation completion from the primary record-content critical path while preserving authorization, ordering, first/previous/next/last semantics, disabled states, return-to behavior, and normal navigation UX. P2 completed the broader detail-page boundary with **PRIMARY CONTENT READY** separated from **FULL ROUTE COMPLETE** using evidence rather than a predetermined layout.
...
**P6 activation:** P6 is **ACTIVE — SCALE-SAFE FINANCIAL READ MODELS** under `G7-SAAS-PERFORMANCE-P6-SCALE-SAFE-FINANCIAL-READ-MODELS-01`. It investigates scale-safe authoritative financial read architecture only: all-pages financial facts, repeated financial-record materialization, aggregation cost, and financial summary/activity contract scalability. It does not pre-authorize a solution, including SQL/RPC, migrations, indexes, caching, materialized views, aggregation tables, infrastructure, or transactional redesign. **GENERIC SHARED SERVICE DETAIL REDESIGN IS NOT AUTOMATICALLY P6; GENERIC QUOTATION DETAIL REDESIGN IS NOT AUTOMATICALLY P6; GENERIC ENTITY-CONTRACT REDESIGN IS NOT AUTOMATICALLY P6; P6 OWNS SCALE-SAFE FINANCIAL READ MODEL WORK ONLY.** Complete totals, balances, summary/activity, authorization, error, and not-found behavior remain authoritative.
...
- **G4 current execution state:** The system-wide diagnostic is complete enough to activate the SaaS Performance & Scale Program above. P0 Customer360 closure, P1 shared record-navigation closeout, P2 detail-page responsiveness architecture, P3 data-access round-trip reduction, P4 related-record UX bounding, and P5 narrow read models / projections are complete. P6 scale-safe financial read models is active for evidence and architecture analysis only. Service Detail's published independent-read concurrency repair and the retained Service/Invoice/Supplier/Payments/J4/J6 return-later queue remain discoverable in `deferred-decisions.md`; they are not silently moved into P6. G4 proceeds measure -> evidence-backed implementation -> verify -> rerank.
...
- Performance/Scale: G4 remains open with completed P0, P1, P2, P3, P4, and P5 publications; remaining work proceeds by evidence-backed measure -> attribute -> repair -> verify -> rerank, beginning with active P6 scale-safe financial read models.
...
- `main`, local HEAD, and `origin/main` are synchronized at this verified baseline (`0 ahead / 0 behind`). The second P5 Customer360 Services projection slice is published at this HEAD; P0-P5 are complete, and P6 is active.
...
- **Immediate current work — P6 ACTIVE:** execute the evidence-led `G7-SAAS-PERFORMANCE-P6-SCALE-SAFE-FINANCIAL-READ-MODELS-01` objective. Examine authoritative financial read scalability without presuming database, caching, infrastructure, or platform changes; generic shared Service/Quotation detail or entity-contract redesign is not P6 by default.
...
- **Delivered product state / current engineering priority:** Feature 007, Feature 008, the latest Quotation/Invoice UX batch, quotation/internal-ABS activation, ABS Void, the Service lifecycle milestone, and P0-P5 performance closures are delivered. The current evidence-backed engineering priority is P6 scale-safe financial read models (`G7-SAAS-PERFORMANCE-P6-SCALE-SAFE-FINANCIAL-READ-MODELS-01`). Feature 009 remains inactive, and Supersede remains deferred.
...
- **Next safe product/engineering direction:** P6 scale-safe financial read models (`G7-SAAS-PERFORMANCE-P6-SCALE-SAFE-FINANCIAL-READ-MODELS-01`) is the active evidence-backed engineering priority. Reports Center remains a separate V1 P1 item and is not the active SaaS Performance & Scale phase; Supersede remains a separate deferred revision workflow. Clerk invitation/webhook smoke remains deferred until production/UAT readiness.
