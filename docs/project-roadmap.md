...
| P9 | ACTIVE — Data-layer attribution | Distinguish Data API/network, PostgREST, PostgreSQL execution, response transfer/serialization, and application orchestration using safe numeric evidence. Do not preselect a query, RPC, index, table, page, route, or optimization target. |
...
- **G4 current execution state:** The system-wide diagnostic is complete enough to activate the SaaS Performance & Scale Program above. P0-P8 are complete, published, and verified. P9 **DATA-LAYER ATTRIBUTION** is active for evidence-led attribution only; no query, RPC, index, table, route, or architectural treatment is presumed. Service Detail's published independent-read concurrency repair and the retained Service/Invoice/Supplier/Payments/J4/J6 return-later queue remain discoverable in `deferred-decisions.md`; they are not silently moved into P9. G4 proceeds measure -> evidence-backed implementation -> verify -> rerank.
...
- Performance/Scale: G4 remains open with completed P0-P8 publications and verification; remaining work proceeds by evidence-backed measure -> attribute -> repair -> verify -> rerank, beginning with active P9 data-layer attribution.
...
### P8 Closeout / P9 Activation Checkpoint — `06b303f3fd082bebf85d38be3faf55dcbf812033`

- P8 is closed at this verified checkpoint. P0-P8 are complete, published, and verified; P9 is active for evidence-led data-layer attribution only.
...
- **Immediate current work — P9 ACTIVE:** **DATA-LAYER ATTRIBUTION** starts with safe numeric evidence across Data API/network, PostgREST, PostgreSQL execution, response transfer/serialization, and application orchestration. No query, RPC, index, table, page, route, or optimization treatment is presumed.
...
- **Delivered product state / current engineering priority:** Feature 007, Feature 008, the latest Quotation/Invoice UX batch, quotation/internal-ABS activation, ABS Void, the Service lifecycle milestone, and P0-P8 performance closures are delivered. The current evidence-backed engineering priority is P9 data-layer attribution. Feature 009 remains inactive, and Supersede remains deferred.
...
- **Next safe product/engineering direction:** P9 data-layer attribution is the active evidence-backed engineering priority. Begin from P9's own attribution contract; do not preselect a database target, query, RPC, index, cache, or optimization. Reports Center remains a separate V1 P1 item and is not the active SaaS Performance & Scale phase; Supersede remains a separate deferred revision workflow. Clerk invitation/webhook smoke remains deferred until production/UAT readiness.
