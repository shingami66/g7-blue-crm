...
|---|---|---|
| P0 | COMPLETE — Customer360 invoice-loading optimization | Published at `7507207c362c641b3c62aa7b1132fb6494fe1c0a`; bounded-preview/complete-facts optimization, regression proof, independent review, and validation cleanup are complete. |
...
| P3 | ACTIVE — Data-access round-trip reduction | Exact active engineering task: `G7-SAAS-PERFORMANCE-P3-DATA-ACCESS-ROUND-TRIP-REDUCTION-01`. Identify material repeated PostgREST reads, serial waves, and safe consolidation opportunities; do not assume a database redesign. |
| P4 | Related-record UX bounding | Customer Detail target is latest 10 contextual invoices plus View All to full Customer-filtered invoice history. This remains planned/documented and was not implemented by P0. Apply equivalent bounding elsewhere only when UX/scale evidence supports it. Preview rows never define financial truth. |
...
**P3 activation:** P3 is now the active **DATA-ACCESS ROUND-TRIP REDUCTION** phase under `G7-SAAS-PERFORMANCE-P3-DATA-ACCESS-ROUND-TRIP-REDUCTION-01`. Existing evidence establishes remote Data API request multiplication and unnecessary round trips as material in specific paths. This activation does not establish PostgreSQL engine slowness or authorize SQL/RPC redesign, indexes, caching, infrastructure/region changes, or framework/data-platform replacement. The preserved data-access conclusion remains **PARTLY — MATERIAL IN SPECIFIC PATHS**.
...
- **G4 current execution state:** The system-wide diagnostic is complete enough to activate the SaaS Performance & Scale Program above. P0 Customer360 closure, P1 shared record-navigation closeout, and P2 detail-page responsiveness architecture are complete: the bounded-preview/complete-facts slice, >preview-boundary financial-activity regression proof, P1 four-family runtime verification, P2 primary/secondary runtime recertification, reviews, publications, and validation cleanup are closed. P3 data-access round-trip reduction is now active. Service Detail's published independent-read concurrency repair and the retained Service/Invoice/Supplier/Payments/J4/J6 return-later queue remain discoverable in `deferred-decisions.md`. G4 proceeds measure -> evidence-backed implementation -> verify -> rerank.
...
- Performance/Scale: G4 remains open with completed P0, P1, and P2 publications; remaining work proceeds by evidence-backed measure -> attribute -> repair -> verify -> rerank, beginning with active P3 data-access round-trip reduction.
...
### Current SaaS Performance & Scale Checkpoint — `b46b9490bf7f739743c47d008c860f892cb49ca3`

- `main`, local HEAD, and `origin/main` are synchronized at this verified baseline (`0 ahead / 0 behind`). The P2 detail-page responsiveness architecture is published at this HEAD; P0, P1, and P2 are complete.
...
- **Immediate current work — P3 ACTIVE:** execute the evidence-led `G7-SAAS-PERFORMANCE-P3-DATA-ACCESS-ROUND-TRIP-REDUCTION-01` objective. Identify material remote request multiplication and unnecessary round trips without presuming PostgreSQL slowness, indexes, SQL/RPC redesign, caching, infrastructure changes, or platform replacement.
