...
- **ABS review/approve complete in current source:** `ABS-MGMT-UI-REVIEW-APPROVE-1` — automated validation passed; PASS by Mozfer manual browser evidence; currently uncommitted and unpushed pending controlled commit/push.
- **Current active implementation task (exactly one):** `ABS-MGMT-FINANCIAL-LIFECYCLE-DESIGN-1`
  - **Purpose:** bounded line-safety review and approval UI using the existing backend contracts.
  - **Mode:** separate implementation gate — no review/approve, void/supersede, or SQL apply in that slice.
- **Locked ABS management order:**
  ...
  6. `ABS-MGMT-VOID-ACTION-1` **blocked** (`ABS_VOID_SUPERSEDE_FINANCIAL_BEHAVIOR_PENDING`)
  7. `ABS-MGMT-SUPERSEDE-ACTION-1` **blocked** (same flag)
  - Optional later: `ABS-MGMT-HISTORY-LIST-1`
- **ABS source-truth:** void/supersede **not** implemented as actions; Create Draft plus draft edit/discard are implemented in source, while review/approve and later write CTAs are not implemented; status enum `draft|approved|voided` only (`superseded` is timestamp, not status); discard + item-edit RPC exceptions exist; active ABS ceiling authoritative when present; approved-quotation fallback when no active scope; historical invoice snapshots immutable.
...
- **Locked V1 critical-path order:** responsive audit → responsive implement + Mozfer re-smoke **PASS** → ABS management design (complete) → ABS read-enrich (complete; pushed) → ABS draft-create UI (complete; pushed) → ABS draft-edit/discard UI (complete; pushed in `df7cf1e9ef9d5302162735bcc87a8aa567385073`) → ABS review/approve (complete in current source; commit/push pending) → **financial lifecycle design/audit** → bounded financial implementation (only after design/audit gates).
...
- `ABS-MGMT-UI-REVIEW-APPROVE-1` (source implemented; automated validation passed; **PASS by Mozfer manual browser evidence**): English-only manual evidence covered Pending review readiness, Safe review, approval confirmation warning, successful activation, disappearance of write controls, and active Service Detail totals. Arabic/English dictionary parity and Arabic wiring are automated-test-covered; no Arabic manual browser evidence or manual rapid double-click test is claimed. Review/Approve commit and push remain separate controlled actions.
...
- `ABS-MGMT-FINANCIAL-LIFECYCLE-DESIGN-1` **next active implementation task** (design/audit gate before bounded financial implementation)
