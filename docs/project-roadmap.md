...
| P1 | ACTIVE — Shared record-navigation critical path | Active engineering task: `G7-SHARED-RECORD-NAVIGATION-CRITICAL-PATH-REMOVAL-01` across Customer, Service, Invoice, and Quotation. The exact repair remains evidence-led. |
...
Program state: **P0 COMPLETE**, **P1 ACTIVE**, and **P2-P10 NOT STARTED / FUTURE PROGRAM PHASES**. P1 is an objective, not a preselected implementation: reduce or remove shared record-navigation work from the primary record-content critical path while preserving authorization, ordering, first/previous/next/last semantics, disabled states, return-to behavior, and normal navigation UX. Evaluation must distinguish **PRIMARY CONTENT READY** from **FULL ROUTE COMPLETE**; actual in-app interaction timing is required when P1 executes.
...
- **G4 current execution state:** The system-wide diagnostic is complete enough to activate the SaaS Performance & Scale Program above. P0 Customer360 closure is complete: the bounded-preview/complete-facts slice, >preview-boundary financial-activity regression proof, review, publication, and validation cleanup are closed. P1 shared record-navigation critical-path work is now active. Service Detail's published independent-read concurrency repair and the retained Service/Invoice/Supplier/Payments/J4/J6 return-later queue remain discoverable in `deferred-decisions.md`. G4 proceeds measure -> evidence-backed implementation -> verify -> rerank.
...
### Current SaaS Performance & Scale Checkpoint — `7507207c362c641b3c62aa7b1132fb6494fe1c0a`

- `main`, local HEAD, and `origin/main` are synchronized at this verified baseline (`0 ahead / 0 behind`). The Customer360 optimization is published at this HEAD and P0 is complete.
...
- **Immediate current work — P1 ACTIVE:** execute the evidence-led `G7-SHARED-RECORD-NAVIGATION-CRITICAL-PATH-REMOVAL-01` objective across Customer, Service, Invoice, and Quotation. Do not preselect a technical solution; distinguish primary-content-ready from full-route-complete timing when the task executes.
...
- **Delivered product state / current engineering priority:** Feature 007, Feature 008, the latest Quotation/Invoice UX batch, quotation/internal-ABS activation, ABS Void, and the Service lifecycle milestone are delivered. P0 Customer360 invoice-loading closeout is complete, and the current evidence-backed engineering priority is P1 shared record-navigation critical-path work (`G7-SHARED-RECORD-NAVIGATION-CRITICAL-PATH-REMOVAL-01`); Feature 009 remains inactive, and Supersede remains deferred.
- **Workspace Location and Governance Rules:**
  ...
  - Canonical branch: `main`; current published checkpoint: `7507207c362c641b3c62aa7b1132fb6494fe1c0a` (local HEAD and `origin/main`, `0 ahead / 0 behind`). The `90adf8faa33c4af1c0049b53817f5f95896a761f` frozen discovery baseline remains historical evidence only.
...
- **Next safe product/engineering direction:** P1 shared record-navigation critical-path work (`G7-SHARED-RECORD-NAVIGATION-CRITICAL-PATH-REMOVAL-01`) is the active evidence-backed engineering priority. Reports Center remains a separate V1 P1 item and is not the active SaaS Performance & Scale phase; Supersede remains a separate deferred revision workflow. Clerk invitation/webhook smoke remains deferred until production/UAT readiness.
