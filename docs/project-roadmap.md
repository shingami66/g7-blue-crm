...
- **W5A Delivery Status:** `CLOSED / COMPLETED`. W5A Expense & Cash Accountability Foundation (`L1-D08-EXPENSE-CASH`) is complete on DEV project `dpddrqjzqohexixgdqiq`.
  ...
  - **Governed RPCs:** 16 atomic mutation RPCs with `SECURITY DEFINER`, fixed search path, request_id replay conflict rejection, and Segregation of Duties (SoD) self-approval rejection.
...
- **Current Product Truth under G7-OD-18:**
  ...
  - **Supplier Quotations** (`supplier_quotations`, `supplier_quotation_lines`) are first-class commercial and operational evidence.
...
- **G4 closure boundary:** The closure covers current-core bounded performance and scale remediation only; it does not certify production performance, full server/database attribution, or SaaS acceptance. Server-Timing/database attribution remains evidence required only if future evidence justifies reopening G4.
- **G2 closure evidence:** G2 Money and Payment Precision is **CLOSED / ENGINEERING COMPLETE**. Accepted evidence confirms exact positive two-decimal SAR precision, pre-mutation rejection of malformed/sub-cent/non-positive/non-finite amounts, preservation of the seven-argument `record_invoice_payment` contract, replay/idempotency and duplicate protection, overpayment protection, deposit-transition and audit behavior, canonical migration reconciliation on DEV/DEMO, read-only verification of active RPC permissions and metadata, and no concrete G2 engineering defect.
...
- **Goals:** G2 is `CLOSED / ENGINEERING COMPLETE` for Money and Payment Precision. G4 is `CLOSED / ENGINEERING COMPLETE` for the current-core bounded performance and scale remediation program. G5 is `CLOSED / ENGINEERING COMPLETE` for Admin Security and Desired-State Mutations. G6 is `CLOSED / ENGINEERING COMPLETE` for Payload and Log Minimization. G7 is `CLOSED / ENGINEERING COMPLETE` for Failure, Health, and Webhook Hardening. G10 engineering remediation is `CLOSED / PUBLISHED / VERIFIED` in `a9d5459` and `9324ce0`; the prior UI-consistency and Final Invoice post-create entries are historical. G3, G8, G9, G11, and G12 remain closed as documented. The current program is current-core quality under completed G2, G4, G5, G6, and G7 gates, not a stale active P9 attribution phase.
...
## 1.2 Historical Review and Remediation Roadmap (superseded by Section 0)

> This section preserves the earlier remediation checkpoint and its evidence. Current Goal, finding, and priority state is owned by Section 0; older “open” entries below are historical unless repeated there.
...
### Historical Checkpoint — Published G3 Closeout (16 August 2026)
...
- **G8 Replay Safety Delivery Progress:** 5 of 5 families complete under locked serial order `Customer -> Service -> Quotation -> Invoice -> Payment`:
  - **Customer Create Replay Safety:** `be96beaebbcfe17432ee0515eef68d4cb6cb19ac` (`feat(g8): add customer create replay safety`) — **COMPLETE / PUBLISHED / DEV-DEMO VERIFIED** (Controller outcome: `PASS WITH WARN`).
  - **Service Create Replay Safety:** `ea61563163fcc73f1a2fdfed1c650807cf8eb77d` (`feat(g8): add service create replay safety`) — **COMPLETE / PUBLISHED / DEV-DEMO VERIFIED** (Controller outcome: `PASS`).
  - **Quotation Create Replay Safety:** `61fd5059e5c735466318d44be7487e71be64708f` (`feat(g8): add quotation create replay safety`) — **COMPLETE / PUBLISHED / FINAL DEV-DEMO STATE RECONCILED** (Controller outcome: `PASS WITH WARN`).
...
### Historical Verified Checkpoint — Manual Regression Results and Owner Decisions (18 August 2026)
...
### Historical active remediation program (superseded by Section 0)
...
|---|---|---|
| P0 | COMPLETE / PUBLISHED / VALIDATED — Customer360 invoice-loading optimization | Published at `7507207c362c641b3c62aa7b1132fb6494fe1c0a`; bounded-preview/complete-facts optimization, regression proof, independent review, and validation cleanup are complete. |
| P1 | COMPLETE / PUBLISHED / VERIFIED — Shared record-navigation critical path | `G7-SHARED-RECORD-NAVIGATION-CRITICAL-PATH-REMOVAL-01` published in `333907264c00ce830e51cb274ee84ad88e08cdaf` across Customer, Service, Invoice, and Quotation. Primary content was successfully decoupled from record navigation; preserved behavior and independent review are recorded in the project status. |
| P2 | COMPLETE / PUBLISHED / VERIFIED — Detail-page responsiveness architecture | `G7-SAAS-PERFORMANCE-P2-DETAIL-PAGE-RESPONSIVENESS-ARCHITECTURE-01` published in `b46b9490bf7f739743c47d008c860f892cb49ca3`. Primary detail content is separated from independent secondary work across Customer, Service, Invoice, and Quotation while authorization, not-found semantics, financial truth, and business-action prerequisites remain preserved. |
| P3 | COMPLETE / PUBLISHED / VERIFIED — Data-access round-trip reduction | Published in `0aed85835501c24d7834401d8cc9f296cc9de5cf` (`perf(customer): reduce Customer360 invoice round trips`). One duplicate invoice access was removed by deriving the bounded preview from the exhaustive ordered paginated invoice-facts result; invoice subflow `3 → 2` remote calls and Customer route topology `8 → 7` reads. |
| P4 | COMPLETE / PUBLISHED / VERIFIED — Related-record UX bounding | `G7-SAAS-PERFORMANCE-P4-RELATED-RECORD-UX-BOUNDING-01` is published in `eca8b431140826851fc2251ba4bcdd9b6a15c908` (`perf(customer): bound Customer360 invoice preview`). Customer Detail previews the latest 10 contextual invoices with View All to the existing customer-filtered Invoice workspace route. Financial facts remain exhaustive and remote reads are unchanged. |
| P5 | COMPLETE / PUBLISHED / VERIFIED — Narrow read models / projections | Service Detail → Related Quotations is published in `3b0ec95cec0b912f84fb4a2f0cdb9578c45a92df` (`perf(service): narrow related quotation projection`): full quotation row plus customer/service relations → route-specific six-field `ServiceQuotationListItem`, one request preserved. Customer360 secondary Services is published in `687371959bb75b0bb80394941c28c86ef478a1c8` (`perf(customer): narrow Customer360 service projection`): `9 → 6` selected fields, one request preserved. Both verdicts: **READ MODEL SUCCESSFULLY NARROWED — PERFORMANCE EFFECT NOT MATERIAL**; authoritative business and financial truth preserved. **NO FURTHER MATERIAL P5 PROJECTION OPPORTUNITY JUSTIFIED.** |
| P6 | COMPLETE / PUBLISHED / VERIFIED — Scale-safe financial read models | Customer360 financial materialization is published in `896e5fd3b8ffdc2b7989915b6cc662bfd22526d7`: complete authoritative history remains processed while previews/activity and application retention are materially bounded. Service Billing is published in `b8e33907be544402b4bc8d8c8ec5b612a2980dff`: two concurrent bounded aggregate RPC reads, O(1) application aggregate state, preserved financial truth/permission separation, authorized DEV/DEMO runtime verification, and **NO ADDITIONAL INDEX JUSTIFIED**. The accepted Service Billing result is **SERVICE BILLING BOUNDED AGGREGATE READ RUNTIME VERIFIED**; low-sample DEV/DEMO timing does not claim production or user-visible latency. |
| P7 | COMPLETE / PUBLISHED / VERIFIED — Large secondary collections | Published supplier-history bound: `9ba1b67942ddeea045dcfd442e2aa690e3431409` (`perf(service): bound supplier history on Service Detail`). Related Quotations reranking found P50 `0`, P95/P99/MAX `1`, and no terminal history; no further material large-collection slice is justified. |
| P8 | COMPLETE / PUBLISHED / VERIFIED — Shared / cold-shell work | Published `06b303f3fd082bebf85d38be3faf55dcbf812033` removed the Business-Year primary-shell barrier through localized server Suspense. Final authorized DEV/DEMO attribution proved app-user request deduplication, negligible effective-locale binding cost, and **P8 ENGINEERING COMPLETE — NO FURTHER MATERIAL SHARED/COLD-SHELL OPPORTUNITY JUSTIFIED**. |
| P9 | HISTORICAL / COMPLETE — Primary-detail performance pass | Complete/published/verified for Customer, Supplier, Service, and Quotation; Invoice was measured and discarded. Later module-entry work is recorded in Section 0. |
| P10 | FUTURE — SaaS performance acceptance / rerank | Re-run critical journeys, primary-ready and full-completion measures, review scale behavior, rerank remaining blockers, and continue until Owner accepts a SaaS baseline. No fixed millisecond threshold is invented here. |

Historical program state (superseded by Section 0): **P0 COMPLETE / PUBLISHED / VALIDATED**, **P1-P8 COMPLETE / PUBLISHED / VERIFIED**, and P9 was then queued for attribution. P1 removed shared record-navigation completion from the primary record-content critical path while preserving authorization, ordering, first/previous/next/last semantics, disabled states, return-to behavior, and normal navigation UX. P2 completed the broader detail-page boundary with **PRIMARY CONTENT READY** separated from **FULL ROUTE COMPLETE** using evidence rather than a predetermined layout.

**P2 closeout:** Published commit `b46b9490bf7f739743c47d008c860f892cb49ca3` (`perf(detail): decouple secondary work from primary detail content`) passed affected focused validation, TypeScript, targeted ESLint, production build, and `git diff --check`. Accepted conclusion: **PRIMARY DETAIL CONTENT SUCCESSFULLY SEPARATED FROM MATERIAL SECONDARY WORK**. Final delegated review returned **ACCEPT — NO FINDINGS** with **REVIEW PASSED — NO BLOCKING FINDINGS**. Final runtime recertification preserved primary-before-secondary readiness for Customer, Service, Invoice, and Quotation, with medians `2007/3139 ms`, `760/929 ms`, `1536/2173 ms`, and `2021/3628 ms` respectively. No quantified pre-P2 improvement is claimed because equivalent historical primary-ready markers did not exist.

**P3 closeout:** P3 is **COMPLETE / PUBLISHED / VERIFIED** in `0aed85835501c24d7834401d8cc9f296cc9de5cf` (`perf(customer): reduce Customer360 invoice round trips`). The Customer 360 invoice path now derives the bounded contextual preview from the same exhaustive ordered paginated invoice-facts result, reducing the invoice subflow from `3 → 2` remote calls and the measured Customer route topology from `8 → 7` reads. Final review was **ACCEPT — NO FINDINGS** with **REVIEW PASSED — NO BLOCKING FINDINGS**; authenticated browser correctness passed; actual-click runtime recertification was **PASS WITH NON-BLOCKING PERFORMANCE LIMITATION**. Comparable medians were baseline `2129/3162/3162 ms` and candidate `2194/3057/3057 ms` for primary/secondary/full completion. The accepted conclusion is **ROUND-TRIP REDUCTION ESTABLISHED WITH LIMITED USER-VISIBLE BENEFIT**. No material user-visible latency improvement, material regression, payload-size conclusion, or memory conclusion is claimed.

**P4 closeout:** P4 is **COMPLETE / PUBLISHED / VERIFIED** in `eca8b431140826851fc2251ba4bcdd9b6a15c908` (`perf(customer): bound Customer360 invoice preview`). The Customer Detail related-invoice preview is bounded to the latest 10 contextual invoices, with View All to the existing customer-filtered Invoice workspace route. Financial facts remain exhaustive, remote reads are unchanged, and the final verdict is **RELATED-RECORD UX SUCCESSFULLY BOUNDED — PERFORMANCE EFFECT NOT MATERIAL**. **FULL FINANCIAL TRUTH PRESERVED INDEPENDENTLY OF PREVIEW BOUND**. Final review was **ACCEPT — NO FINDINGS** with **REVIEW PASSED — NO BLOCKING FINDINGS**; browser verification passed.

**P5 closeout:** P5 is **COMPLETE / PUBLISHED / VERIFIED**. `3b0ec95cec0b912f84fb4a2f0cdb9578c45a92df` narrowed Service Detail → Related Quotations from a full quotation row plus customer/service relations to a six-field route-specific projection; `687371959bb75b0bb80394941c28c86ef478a1c8` narrowed Customer360 secondary Services from `9 → 6` fields. Both retained one remote request with filters, ordering, and cardinality unchanged; both accepted verdicts are **READ MODEL SUCCESSFULLY NARROWED — PERFORMANCE EFFECT NOT MATERIAL** and **AUTHORITATIVE BUSINESS AND FINANCIAL TRUTH PRESERVED**. No measured payload-byte or latency result is claimed. The final evidence-backed assessment is **NO FURTHER MATERIAL P5 PROJECTION OPPORTUNITY JUSTIFIED** and **P5 NARROW READ MODEL / PROJECTION ENGINEERING COMPLETE**. Low-value Customer360 quotation scalar trimming is not mandatory work; Customer360 invoices retain authoritative financial completeness; Customer360 payments are already appropriately narrow; generic shared detail contracts remain future/evidence-gated.

**P6 closeout:** P6 is **COMPLETE / PUBLISHED / VERIFIED**. Customer360 financial materialization preserves exhaustive authoritative invoice history while materially bounding previews/activity and application retention. Service Billing now uses exactly two concurrent bounded aggregate RPC reads; result cardinality and application aggregate state are bounded, PostgreSQL performs financial aggregation, financial truth and permission separation are preserved, and authority/exposure failure behavior remains fail-closed. The Service Billing migration/security shape and small/1,000-invoice structural behavior were verified in authorized DEV/DEMO; the synthetic fixture was atomically removed; query-plan evidence is **NO ADDITIONAL INDEX JUSTIFIED**. The accepted results are **SERVICE BILLING BOUNDED AGGREGATE READ RUNTIME VERIFIED** and **STRUCTURAL SCALE-SAFETY IMPROVEMENT VERIFIED — LATENCY EVIDENCE INCONCLUSIVE**. No production or user-visible latency claim is made.

**Historical P8 closeout / P9 activation (superseded by Section 0):** P8 is **COMPLETE / PUBLISHED / VERIFIED**. Published `06b303f3fd082bebf85d38be3faf55dcbf812033` preserved Business-Year acquisition semantics while moving selector data behind localized server Suspense; accepted DEV/DEMO evidence changed layout shell-ready median from `1.270 s` to `603.1 ms`, with Business-Year beginning approximately `69-72 ms` after shell readiness and all five reads still concurrent. Final authorized root auth/locale attribution found exactly one request-deduplicated app-user execution per trace, negligible effective-locale session-binding cost, and a remaining `app_users` prerequisite read that is necessary for active-user, role, and persisted-locale correctness. **P8 ENGINEERING COMPLETE — NO FURTHER MATERIAL SHARED/COLD-SHELL OPPORTUNITY JUSTIFIED.** The former P9 attribution activation is historical; Section 0 records its completion without a preselected target or treatment.

P0 closeout: the published Customer360 invoice-loading optimization at `7507207c362c641b3c62aa7b1132fb6494fe1c0a` (`perf(customer): optimize Customer360 invoice loading`) is self-contained in `src/lib/customer-360/queries.ts` and `src/lib/customer-360/queries.test.ts`. Fresh exact-HEAD plain-snapshot validation passed Customer360 focused tests 14/14, TypeScript, targeted lint, and production build. Independent full-file review returned **ACCEPT — NO FINDINGS**; the Owner manually removed the temporary validation snapshot, so P0 validation cleanup is complete.
...
### Historical remediation waiting on decisions / evidence (superseded by Section 0)

> The following decisions, findings, and gates preserve the earlier checkpoint; Section 0 is the current source of truth.
...
- **Historical G4 execution state (superseded by Section 0):** The system-wide diagnostic was complete enough to activate the SaaS Performance & Scale Program above. P0-P8 were complete, published, and verified. The former P9 attribution queue was evidence-led only; no query, RPC, index, table, route, or architectural treatment was presumed. Service Detail's published independent-read concurrency repair and the retained Service/Invoice/Supplier/Payments/J4/J6 return-later queue remain discoverable in `deferred-decisions.md`; they were not silently moved into P9. G4 proceeds measure -> evidence-backed implementation -> verify -> rerank.
...
- **Historical open remediation findings at this checkpoint:** G10 UI consistency remediation (oversized buttons, actions, pagination); Final Invoice post-create defect (root cause unclassified; requires diagnosis of post-create state, invoice existence, eligibility refresh, billing-state reconciliation, duplicate protection, and error mapping). Section 0 records their current closure state.
...
- **Historical gates at this checkpoint:** G12 was CLOSED / PUBLISHED / CANONICALLY VERIFIED; G3 and G9 were OWNER ACCEPTED; Security, financial integrity, Database/Migration, Performance/Scale, Release, and remaining engineering closeouts were blocked or pending the listed open Goals and evidence. Section 0 records the current G10 closure and remaining gates; production sign-off is not complete.
...
- Performance/Scale: G4 remains open with the completed primary-detail and bounded module-entry checkpoints in Section 0; remaining work proceeds by evidence-backed measure -> attribute -> repair -> verify -> rerank.
...
### Historical P8 Closeout / P9 Activation Checkpoint — `06b303f3fd082bebf85d38be3faf55dcbf812033`

- P8 is closed at this verified checkpoint. P0-P8 are complete, published, and verified; the P9 activation wording is historical and superseded by Section 0.
...
- **P0 closeout:** the published bounded-preview/complete-facts implementation passed fresh exact-HEAD plain-snapshot validation (Customer360 focused tests 14/14, TypeScript, targeted lint, and production build), independent review returned **ACCEPT — NO FINDINGS**, and Owner cleanup of the temporary validation snapshot is complete.
- **P1 closeout:** `G7-SHARED-RECORD-NAVIGATION-CRITICAL-PATH-REMOVAL-01` is complete, published, and verified across Customer, Service, Invoice, and Quotation. Phase-aware evidence demonstrated primary content can become ready before secondary record navigation; delegated independent review returned **ACCEPT — NO FINDINGS**. This does not reinterpret record navigation as intrinsically fast.
- **P2 closeout:** `G7-SAAS-PERFORMANCE-P2-DETAIL-PAGE-RESPONSIVENESS-ARCHITECTURE-01` is complete, published, and runtime-recertified in `b46b9490bf7f739743c47d008c860f892cb49ca3`. Final delegated review returned **ACCEPT — NO FINDINGS** with **REVIEW PASSED — NO BLOCKING FINDINGS**.
- **P7 closeout:** `G7-SAAS-PERFORMANCE-P7-LARGE-SECONDARY-COLLECTIONS-01` is complete, published, and verified. The supplier-history bound is published in `9ba1b67942ddeea045dcfd442e2aa690e3431409`; Related Quotations was not material at authorized DEV/DEMO scale (P50 `0`, P95/P99/MAX `1`, no terminal historical quotations). No further large-secondary-collection implementation is justified.
- **Historical immediate work:** P9 attribution was to start with safe numeric evidence across Data API/network, PostgREST, PostgreSQL execution, response transfer/serialization, and application orchestration. No query, RPC, index, table, page, route, or optimization treatment was presumed.
...
- **Historical delivered product state / engineering priority (superseded by Section 0):** Feature 007, Feature 008, the latest Quotation/Invoice UX batch, quotation/internal-ABS activation, ABS Void, the Service lifecycle milestone, and P0-P8 performance closures are delivered. The former evidence-backed priority was P9 data-layer attribution. Feature 009 remains inactive, and Supersede remains deferred.
- **Workspace Location and Governance Rules:**
  ...
  - Canonical branch: `main`; published P7 checkpoint: `9ba1b67942ddeea045dcfd442e2aa690e3431409`. The `90adf8faa33c4af1c0049b53817f5f95896a761f` frozen discovery baseline remains historical evidence only.
...
- **Historical next safe product/engineering direction (superseded by Section 0):** P9 data-layer attribution was the evidence-backed priority at this checkpoint. Its attribution contract did not preselect a database target, query, RPC, index, cache, or optimization. Reports Center remains a separate V1 P1 item and is not the active SaaS Performance & Scale phase; Supersede remains a separate deferred revision workflow. Clerk invitation/webhook smoke remains deferred until production/UAT readiness.
