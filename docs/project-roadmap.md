...
| P7 | ACTIVE — Large secondary collections | Exact active task: `G7-SAAS-PERFORMANCE-P7-LARGE-SECONDARY-COLLECTIONS-01`. Begin with current evidence, cardinality/use analysis, and prioritization of Service Quotations, allocations, bookings, and other high-cardinality secondary collections. Bounded preview, pagination, workspace separation, and narrower read models are candidate treatments, not selected solutions. |
...
**P7 activation:** P7 is **ACTIVE — LARGE SECONDARY COLLECTIONS** under `G7-SAAS-PERFORMANCE-P7-LARGE-SECONDARY-COLLECTIONS-01`. It begins with current evidence, cardinality/use analysis, and prioritization for Service Quotations, allocations, bookings, and other high-cardinality secondary collections. No treatment or implementation is preselected.
...
- **G4 current execution state:** The system-wide diagnostic is complete enough to activate the SaaS Performance & Scale Program above. P0 Customer360 closure, P1 shared record-navigation closeout, P2 detail-page responsiveness architecture, P3 data-access round-trip reduction, P4 related-record UX bounding, P5 narrow read models / projections, and P6 scale-safe financial read models are complete. P7 large secondary collections is active for evidence, cardinality/use analysis, and prioritization only. Service Detail's published independent-read concurrency repair and the retained Service/Invoice/Supplier/Payments/J4/J6 return-later queue remain discoverable in `deferred-decisions.md`; they are not silently moved into P7. G4 proceeds measure -> evidence-backed implementation -> verify -> rerank.
...
- Performance/Scale: G4 remains open with completed P0, P1, P2, P3, P4, P5, and P6 publications; remaining work proceeds by evidence-backed measure -> attribute -> repair -> verify -> rerank, beginning with active P7 large secondary collections.
...
### Current SaaS Performance & Scale Checkpoint — `687371959bb75b0bb80394941c28c86ef478a1c8`

- `main`, local HEAD, and `origin/main` are synchronized at this verified baseline (`0 ahead / 0 behind`). P0-P6 are complete, and P7 is active for evidence/cardinality analysis only.
...
- **Immediate current work — P7 ACTIVE:** execute the evidence-led `G7-SAAS-PERFORMANCE-P7-LARGE-SECONDARY-COLLECTIONS-01` objective. Examine Service Quotations, allocations, bookings, and other high-cardinality secondary collections through current evidence, cardinality/use analysis, and prioritization; no database, caching, infrastructure, or platform solution is presumed.
...
- **Delivered product state / current engineering priority:** Feature 007, Feature 008, the latest Quotation/Invoice UX batch, quotation/internal-ABS activation, ABS Void, the Service lifecycle milestone, and P0-P6 performance closures are delivered. The current evidence-backed engineering priority is P7 large secondary collections (`G7-SAAS-PERFORMANCE-P7-LARGE-SECONDARY-COLLECTIONS-01`). Feature 009 remains inactive, and Supersede remains deferred.
- **Workspace Location and Governance Rules:**
  ...
  - Canonical branch: `main`; current published checkpoint: `687371959bb75b0bb80394941c28c86ef478a1c8` (local HEAD and `origin/main`, `0 ahead / 0 behind`). The `90adf8faa33c4af1c0049b53817f5f95896a761f` frozen discovery baseline remains historical evidence only.
...
- **Next safe product/engineering direction:** P7 large secondary collections (`G7-SAAS-PERFORMANCE-P7-LARGE-SECONDARY-COLLECTIONS-01`) is the active evidence-backed engineering priority. Reports Center remains a separate V1 P1 item and is not the active SaaS Performance & Scale phase; Supersede remains a separate deferred revision workflow. Clerk invitation/webhook smoke remains deferred until production/UAT readiness.
