...
| P5 | ACTIVE — Narrow read models / projections | Exact next engineering task: `G7-SAAS-PERFORMANCE-P5-NARROW-READ-MODELS-AND-PROJECTIONS-01`. Audit material `select("*")`, broad joins, and route projections with field-use verification, focused tests, and independent review; no broad schema, database, or platform change is authorized by this activation. |
...
Program state: **P0 COMPLETE / PUBLISHED / VALIDATED**, **P1 COMPLETE / PUBLISHED / VERIFIED**, **P2 COMPLETE / PUBLISHED / VERIFIED**, **P3 COMPLETE / PUBLISHED / VERIFIED**, **P4 COMPLETE / PUBLISHED / VERIFIED**, **P5 ACTIVE**, and **P6-P10 NOT STARTED / FUTURE PROGRAM PHASES**. P1 removed shared record-navigation completion from the primary record-content critical path while preserving authorization, ordering, first/previous/next/last semantics, disabled states, return-to behavior, and normal navigation UX. P2 completed the broader detail-page boundary with **PRIMARY CONTENT READY** separated from **FULL ROUTE COMPLETE** using evidence rather than a predetermined layout.
...
**P5 activation:** P5 is now the active **NARROW READ MODELS / PROJECTIONS** phase under `G7-SAAS-PERFORMANCE-P5-NARROW-READ-MODELS-AND-PROJECTIONS-01`. Begin with field-use verification for material `select("*")`, broad joins, and route projections, then validate focused tests and independent review. This activation does not authorize broad schema, database, caching, infrastructure, or platform changes.
...
- **G4 current execution state:** The system-wide diagnostic is complete enough to activate the SaaS Performance & Scale Program above. P0 Customer360 closure, P1 shared record-navigation closeout, P2 detail-page responsiveness architecture, P3 data-access round-trip reduction, and P4 related-record UX bounding are complete: the bounded-preview/complete-facts slice, >preview-boundary financial-activity regression proof, P1 four-family runtime verification, P2 primary/secondary runtime recertification, P3 duplicate-read reduction, P4 latest-10 preview/View All boundary, reviews, publications, and validation cleanup are closed. P5 narrow read models / projections is now active. Service Detail's published independent-read concurrency repair and the retained Service/Invoice/Supplier/Payments/J4/J6 return-later queue remain discoverable in `deferred-decisions.md`. G4 proceeds measure -> evidence-backed implementation -> verify -> rerank.
...
- Performance/Scale: G4 remains open with completed P0, P1, P2, P3, and P4 publications; remaining work proceeds by evidence-backed measure -> attribute -> repair -> verify -> rerank, beginning with active P5 narrow read models / projections.
...
### Current SaaS Performance & Scale Checkpoint — `eca8b431140826851fc2251ba4bcdd9b6a15c908`

- `main`, local HEAD, and `origin/main` are synchronized at this verified baseline (`0 ahead / 0 behind`). The P4 related-record UX bounding closure is published at this HEAD; P0, P1, P2, P3, and P4 are complete, and P5 is active.
...
- **Immediate current work — P5 ACTIVE:** execute the evidence-led `G7-SAAS-PERFORMANCE-P5-NARROW-READ-MODELS-AND-PROJECTIONS-01` objective. Audit material projections and broad joins with field-use verification, focused tests, and independent review, without presuming database, caching, infrastructure, or platform changes.
...
- **Delivered product state / current engineering priority:** Feature 007, Feature 008, the latest Quotation/Invoice UX batch, quotation/internal-ABS activation, ABS Void, the Service lifecycle milestone, P0 Customer360 invoice-loading closeout, P1 shared record-navigation closeout, P2 detail-page responsiveness architecture, P3 data-access round-trip reduction, and P4 related-record UX bounding are delivered. The current evidence-backed engineering priority is P5 narrow read models / projections (`G7-SAAS-PERFORMANCE-P5-NARROW-READ-MODELS-AND-PROJECTIONS-01`). Feature 009 remains inactive, and Supersede remains deferred.
- **Workspace Location and Governance Rules:**
  ...
  - Canonical branch: `main`; current published checkpoint: `eca8b431140826851fc2251ba4bcdd9b6a15c908` (local HEAD and `origin/main`, `0 ahead / 0 behind`). The `90adf8faa33c4af1c0049b53817f5f95896a761f` frozen discovery baseline remains historical evidence only.
...
- **Next safe product/engineering direction:** P5 narrow read models / projections (`G7-SAAS-PERFORMANCE-P5-NARROW-READ-MODELS-AND-PROJECTIONS-01`) is the active evidence-backed engineering priority. Reports Center remains a separate V1 P1 item and is not the active SaaS Performance & Scale phase; Supersede remains a separate deferred revision workflow. Clerk invitation/webhook smoke remains deferred until production/UAT readiness.
