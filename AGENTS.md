...
- When a Writer introduces or materially changes a scale-sensitive collection path, the independent Reviewer explicitly checks the collection classification, the server-side bound or a justified bounded/exhaustive exception, list projection versus detail projection, enrichment scope, obvious N+1 or repeated collection-scan patterns, and focused regression protection when the scale contract is material. An unbounded growing operational collection is a MATERIAL finding unless repository evidence supports a concrete bounded or exhaustive justification. Runtime benchmarking is not required to report a deterministic structural scale-safety violation.
...
- Use `$g7-nextjs-performance-engineering` when a task materially concerns reported page or navigation slowness, a performance regression, measurable latency, payload, bundle, rendering, performance-sensitive loading, optimization and remeasurement, or new/modified code that materially introduces or changes a scale-sensitive collection surface such as a global/growing operational list, search/result collection, history/activity feed, large or growing picker/chooser, dashboard collection read, or collection-enrichment pipeline. Do not route it for ordinary presentation-only React/TSX changes or provably bounded trivial collections.
- Use `$g7-nextjs-framework-engineering` when a task materially requires Next.js framework-mechanism reasoning—such as App Router boundaries, Server Actions, Route Handlers, Proxy, routing, streaming, rendering, caching, configuration, or version-sensitive APIs; do not route it for ordinary React/TSX work.
- Use `$g7-supabase-data-engineering` when a task materially requires Supabase-specific data-access or trust-boundary reasoning—such as browser/server/admin client choice, Data API queries, service-role/RLS boundaries, typed Database access, RPC contracts, or materially relevant Supabase products; do not route it for ordinary React/TSX work or incidental persistence.
- Use `$g7-erp-design-guard` for material UI, UX, accessibility, RTL/LTR, or visual work; also route material work on tables, metrics, reports, Arabic UI, mixed-language text, money/numbers, business identifiers, dates, pagination, directional navigation/icons, or responsive tabular/list surfaces to `$g7-rtl-bidi-guard` (`.agents/skills/g7-rtl-bidi-guard/SKILL.md`). Use `$g7-speckit-plan-guard` for Spec Kit planning.
