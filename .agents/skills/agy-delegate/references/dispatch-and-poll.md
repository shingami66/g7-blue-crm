...
| `--effort <value>` | Explicit reasoning effort passed once to `agy`; accepted values are `low`, `medium`, and `high`, with no fallback. |
...
| `--exclude-path <path>` | Exact repo-relative path omitted from Git status, diff, and fingerprint hashing. Repeatable; glob syntax and parent traversal are rejected. |
...
- `effort` - the exact requested reasoning effort passed to `agy`, or `null` when omitted
- `excludePaths` - normalized exact repo-relative paths omitted from Git inspection
...
```bash
agy --new-project --add-dir <repo> --model <model> --effort high --print-timeout 30m --print=<brief>
agy --continue --model <model> --effort high --print-timeout 30m --print=<delta brief>
agy --conversation <id> --model <model> --effort high --print-timeout 30m --print=<delta brief>
```
