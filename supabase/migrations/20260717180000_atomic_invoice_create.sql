-- Atomic Invoice Creation RPC (Deposit / Final create path)
-- Canonical contract: docs/atomic-invoice-creation-contract.md
--
-- IMPORTANT:
--   - DEV/DEMO only. Do NOT apply automatically.
--   - Application requirePermission(invoices:write) remains mandatory before service_role call.
...
BEGIN
    ...
    IF p_invoice_type = 'deposit' THEN
        ...
        FROM public.invoices i
        WHERE i.service_id = p_service_id
          AND i.approved_quotation_id = p_quotation_id
...
'App must call requirePermission(invoices:write) before invoke. DEV/DEMO contract: '
'docs/atomic-invoice-creation-contract.md. Not production-ready.';
