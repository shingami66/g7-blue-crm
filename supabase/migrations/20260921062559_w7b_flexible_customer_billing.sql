-- W7B: Flexible customer billing / progress invoicing.
-- Repository migration source only. Do not apply without explicit environment authority.
...
ALTER TABLE public.invoices
    ...
    CHECK (invoice_type IN ('deposit', 'progress', 'final')) NOT VALID;

CREATE TABLE IF NOT EXISTS public.invoice_draft_update_mutations (
    ...
    CONSTRAINT invoice_draft_update_mutations_key_unique UNIQUE (invoice_id, mutation_key)
);

ALTER TABLE public.invoice_draft_update_mutations ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.invoice_draft_update_mutations
FROM PUBLIC, anon, authenticated, service_role;

-- Active ABS snapshots preserve authority metadata while progress invoices use
-- a bounded payment line matching the requested amount.
...
-- Draft amount and due-date corrections are allowed only while the invoice
-- remains a draft. Issued financial facts, authority links, and snapshots stay immutable.
...
BEGIN
    RETURN jsonb_build_object(
        ...
        'requested_amount', CASE
            WHEN p_invoice_type IN ('deposit', 'progress') AND p_requested_amount IS NOT NULL
...
-- Flexible create is separate from the legacy deposit/final RPC so existing
-- signatures and replay behavior remain stable for historical callers.
...
-- Only the server-side action may invoke the flexible financial RPC.
...
-- Guarded draft correction for flexible invoices. The service lock serializes
-- the exposure check with concurrent creates and the trigger rechecks the ceiling.
