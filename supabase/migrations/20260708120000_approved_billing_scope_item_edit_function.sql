-- Approved Billing Scope Item Edit Atomic Function (DRAFT ONLY)
-- Purpose:
--   1. Provide a narrow transactional safety exception for draft item edit only.
--   2. Update a single item within a draft approved billing scope and recalculate header totals atomically.
...
BEGIN
    ...
    UPDATE public.approved_billing_scope_items
    SET
        decision = p_decision,
    ...
    SELECT
        coalesce(sum(accepted_subtotal), 0),
        coalesce(sum(accepted_vat_amount), 0),
        coalesce(sum(accepted_grand_total), 0)
    INTO
        v_new_subtotal,
        v_new_vat_amount,
        v_new_grand_total
    FROM public.approved_billing_scope_items
    WHERE approved_billing_scope_id = p_scope_id;

    UPDATE public.approved_billing_scopes
    SET
        accepted_subtotal = v_new_subtotal,
        accepted_vat_amount = v_new_vat_amount,
        accepted_grand_total = v_new_grand_total,
        line_safety_status = CASE WHEN v_is_material THEN 'pending_review' ELSE line_safety_status END,
        updated_at = now()
    WHERE id = p_scope_id;
