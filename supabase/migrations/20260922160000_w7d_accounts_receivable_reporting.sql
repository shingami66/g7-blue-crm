-- W7D Accounts Receivable Reporting: current and historical as-of reconstruction.
--
-- Source-only migration draft.  It is intentionally not applied by this task.
-- The function keeps invoice gross, W7A settlement, W7C corrections and
-- customer cash semantics separate and returns only a bounded drill page.
...
BEGIN
    IF to_regclass('public.invoices') IS NULL
        ...
        OR to_regclass('public.customer_receipt_reversals') IS NULL
        OR to_regclass('public.customer_internal_credit_adjustments') IS NULL
        OR to_regclass('public.customer_internal_credit_adjustment_reversals') IS NULL
        OR to_regclass('public.customer_credit_applications') IS NULL
        OR to_regclass('public.customer_credit_application_reversals') IS NULL
    THEN
        RAISE EXCEPTION USING MESSAGE = 'w7d_required_foundation_missing';
    END IF;

    IF to_regprocedure('public.get_accounts_receivable_report(date,date,date,integer,integer)') IS NOT NULL
    THEN
        RAISE EXCEPTION USING MESSAGE = 'w7d_report_function_already_exists';
    END IF;
END;
$$;

CREATE INDEX customer_ar_reporting_invoices_issue_due_idx
    ON public.invoices (issued_at, due_date, id)
    WHERE COALESCE(is_deleted, false) = false
      AND issued_at IS NOT NULL
      AND status NOT IN ('draft', 'cancelled', 'voided');

CREATE INDEX customer_ar_reporting_payments_date_idx
    ON public.payments (date, id)
    WHERE COALESCE(is_deleted, false) = false
...
BEGIN
    ...
    SELECT
        ...
        COUNT(d.id)::bigint,
        COALESCE((SELECT detail_rows FROM detail_page), '[]'::jsonb),
        cp.customer_count,
        cp.customer_rows
    FROM params p
